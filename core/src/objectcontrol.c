				...
				if (receivedMONRData > 0 && getISOMessageType(buffer, receivedMONRData, 0) == MESSAGE_ID_MONR) {
					...
					if (decodeMONRMessage
						...
						timersub(&monitorData.lastDataUpdate, &monitorData.MonrData.timestamp,
								 &monitorDataAge);
						if (monitorDataAge.tv_sec || monitorDataAge.tv_usec > (long)(MAX_NETWORK_DELAY_USEC)) {
							LogMessage(LOG_LEVEL_WARNING, "Network delay from object %u exceeds 100 ms (%ld ms delay)",
