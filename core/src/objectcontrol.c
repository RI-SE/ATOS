			...
			else if (iCommand == COMM_STRT && (vGetState() == OBC_STATE_ARMED) /*|| OBC_STATE_INITIALIZED) */ )	//OBC_STATE_INITIALIZED is temporary!
			{
				...
				if (iFindObjectsInfo
					...
					for (iIndex = 0; iIndex < nbr_objects; iIndex++) {
						...
						//Reset direct control action data 
						DataDictionaryResetRequestedControlAction(object_transmitter_ids[iIndex]);
					}
				...
				ASPDebugRate = 1;
				DataDictionaryGetVOILReceiversStr(VOILReceivers, SMALL_BUFFER_SIZE_254);
				DataDictionaryGetDTMReceiversStr(DTMReceivers, SMALL_BUFFER_SIZE_254);
			}
