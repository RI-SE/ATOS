...
U32 SimulatorControlBuildObjectMonitorMessage(C8 * MessageBuffer, C8 * MONRData,
											  ObjectMonitorType * ObjectMonitorData, U8 debug);
...
int simulatorcontrol_task(TimeType * GPSTime, GSDType * GSD) {
	...
	if (SimulatorIpU32 != 0)	//Do stuff if IP is defined
	{
		...
		while (GPSTime->TimeInitiatedU8 == 0) {
			usleep(100);
		}
		...
		while (!iExit) {
			if (SimulatorTCPSocketfdI32 <= 0) {
				ClientResultI32 =
					UtilConnectTCPChannel("SimulatorControl", &SimulatorTCPSocketfdI32,
										  (const C8 *)SimulatorServerIPC8, SimulatorTCPPortU16);
				UtilCreateUDPChannel("SimulatorControl", &SimulatorUDPSocketfdI32,
									 (const C8 *)SimulatorServerIPC8, SimulatorUDPPortU16, &simulator_addr);
			}
			else {
				...
				if (SimulatorInitiatedReqU8 == 0 && SimulatorTCPSocketfdI32 > 0) {
					SimulatorControlInitSimulator(&SimulatorTCPSocketfdI32, SupervisorModeU8,
												  SimFuncReqResponse, 0);
					SimulatorInitiatedReqU8 = 1;
				}

				//Send ObjectAddressList if in correct mode and ObjectControl is in armed state
				if (OBCStateStatus == OBC_STATE_ARMED
					&& (SMGD.SimulatorModeU8 == SIM_CONTROL_DTM_MODE
						|| SMGD.SimulatorModeU8 == SIM_CONTROL_VIM_DTM_MODE)
					&& ObjectAddressListSentU8 == 0) {
					SimulatorControlObjectAddressList(&SimulatorTCPSocketfdI32,
													  DTMObjectAddressesC8 /*"192.168.0.15" */ ,
													  SimFuncReqResponse, 0);
				...
				if (ClientResultI32 > 0 || WaitAllDataU8 == 1) {
					...
					if (ClientResultI32 > 0 && SIM_CONTROL_DEBUG_TCP_RX_DATA) {
						...
						for (int i = 0; i < ClientResultI32; i++)
							printf("%x ", (C8) RxBuffer[i]);
						printf("\n");
					...
					if (RxTotalDataU32 != ReqRxLengthU32) {
						WaitAllDataU8 = 1;
						DataChunkedU8 = 1;
						for (i = 0; i < ClientResultI32; i++, j++)
							ReceiveBuffer[j] = RxBuffer[i];
					}
					else {
						if (DataChunkedU8 == 1)
							for (i = 0; i < ClientResultI32; i++, j++)
								ReceiveBuffer[j] = RxBuffer[i];
						else if (DataChunkedU8 == 0)
							for (j = 0; j < RxTotalDataU32; j++)
								ReceiveBuffer[j] = RxBuffer[j];
						DataChunkedU8 = 0;
						ReqRxLengthU32 = 0;
						WaitAllDataU8 = 0;
					}
				...
				if (WaitAllDataU8 == 0 && ClientResultI32 > 0) {
					if (SIM_CONTROL_DEBUG_TCP_RX_DATA) {
						...
						for (int i = 0; i < RxTotalDataU32; i++)
					...
					if (SimRxCodeU16 > 0) {
						...
						if (SimRxCodeU16 == 0x7E7E && SMGD.SimulatorModeU8 == SIM_CONTROL_DTM_MODE) {
							...
							SimulatorControlBinaryMessageManager(RxTotalDataU32, ReceiveBuffer, MsgQueBuffer,
																 0);
							...
							if (SupervisorIpU32 == 0) {
								...
								for (i = 0; i < strlen(MsgQueBuffer); i++)
									GSD->Chunk[i] = MsgQueBuffer[i];
							...
							bzero(SimFuncRx, SIM_CONTROL_BUFFER_SIZE_64);
							strncpy(SimFuncRx, ReceiveBuffer + 6,
									(U64) strchr(ReceiveBuffer + 6, ':') - (U64) (ReceiveBuffer + 6));
							...
							if (strcmp(SimFuncReqResponse, SimFuncRx) == 0 && SimRxCodeU16 == 1) {
								...
								if (strcmp(SimFuncReqResponse, "InitSimulator") == 0) {
									...
									if (ReceiveBuffer[ResponseDataIndexU16] >= SIM_CONTROL_VIM_MODE
										&& ReceiveBuffer[ResponseDataIndexU16] <= SIM_CONTROL_DEBUG_MODE)
				...
				UtilReceiveUDPData("SimulatorControl", &SimulatorUDPSocketfdI32, UDPReceiveBuffer, 100,
								   &ReceivedNewData, 0);
			...
			if (iCommand == COMM_EXIT) {
				...
				LengthU32 =
					SimulatorControlBuildObjectMonitorMessage(SendBuffer, MqRecvBuffer, &ObjectMonitorData,
															  0);
				UtilSendUDPData("SimulatorControl", &SimulatorUDPSocketfdI32, &simulator_addr, SendBuffer,
								LengthU32, 0);
			}
			else if (iCommand == COMM_OBC_STATE) {
				OBCStateStatus = (U8) * MqRecvBuffer;
				//Prepare to send ObjectAddressList
				if (OBCStateStatus == OBC_STATE_CONNECTED)
					ObjectAddressListSentU8 = 0;
			}


			++CycleU16;
			if (CycleU16 >= SIM_CONTROL_HEARTBEAT_TIME_MS / SIM_CONTROL_TASK_PERIOD_MS)
				CycleU16 = 0;
			sleep_time.tv_sec = 0;
...
U32 SimulatorControlBuildObjectMonitorMessage(C8 * MessageBuffer, C8 * MONRData,
	...
	for (i = 0; i < sizeof (ObjectMonitorType); i++)
		*(MessageBuffer + i + 4) = *ptr++;
	...
	*(MessageBuffer + 3) = (U8) (i);
	...
	if (debug) {
		...
		for (j = 0; j < sizeof (ObjectMonitorType) + 4; j++)
			printf("%x ", (unsigned char)MessageBuffer[j]);
		printf("\n");
	}
...
U32 SimulatorControlBinaryMessageManager(I32 DataLength, C8 * ReceiveBuffer, C8 * MsgQueBuffer, U8 Debug) {
	...
	if (Debug) {
		...
		for (j = 80; j < DataLength; j++)
			printf("%x ", (C8) ReceiveBuffer[j]);	//j = 80 is where first timestamp is
		printf("\n");
	}
