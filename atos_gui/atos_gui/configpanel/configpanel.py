...
class ConfigPanelNode(Node):
    ...
    def __init__(self) -> None:
        ...
        time.sleep(
            0.5
        )  # Allow time for this node to initialize before discovering other nodes, otherwise it will fail to find the others.
        self.active_nodes_and_namespaces = self.get_node_names_and_namespaces()
        self.active_node_list = [
            node
            for node, namespace in self.active_nodes_and_namespaces
            if node in modules and "atos" in namespace
        ]
        ...
        self.parameters = {}
        threading.Thread(
            target=self.fetch_parameters_list, args=(self.parameter_clients,)
        ).start()
                    ...
                    with ui.tab_panels(tabs, value="Home"):
                        with ui.tab_panel("Home"):
                            ...
                            if self.active_node_list:
                                ui.label(
                                    "Select a node to configure. Press the refresh button if you can't find the node you're looking for."
                                )
                            else:
                            ...
                            ui.button(
                                "Refresh", on_click=lambda: self.refresh(), icon="🔄"
                            )
                        for node in self.active_node_list:
                            with ui.tab_panel(node.replace("_", " ")):
                                for param_name in self.json_schema[
                                    "modules"
                                ][
                                    node
                                ][
                                    "ros__parameters"
                                ]:  # Should this loop through the active parameters instead?
                                    param_text = param_name.replace("_", " ")
                                    param_type = self.json_schema["modules"][node][
                                        "ros__parameters"
                                    ][param_name]["type"]
                                    param_description = self.json_schema["modules"][
                                        node
                                    ]["ros__parameters"][param_name]["description"]
                                    with ui.row():
                                        match param_type:
                                            case "file":
                                                with ui.column():
                                                    ui.button(
                                                        f"Select {param_name.replace('_', ' ')}:",
                                                        on_click=lambda node=node,
                                                        param_name=param_name: self.pick_file(
                                                            node, param_name
                                                        ),
                                                        icon="📂",
                                                    )
                                            case "bool":
                                                ui.checkbox(param_text).bind_value(
                                                    self.parameters, param_name
                                                ).on(
                                                    "change",
                                                    lambda result,
                                                    node=node,
                                                    param_name=param_name: self.set_parameter(
                                                        node, param_name, result.value
                                                    ),
                                                )
                                            case "int":
                                                ui.number(param_text).bind_value(
                                                    self.parameters, param_name
                                                ).on(
                                                    "keydown.enter",
                                                    lambda result,
                                            ...
                                            case "string":
                                                ui.input(param_text).bind_value(
                                                    self.parameters, param_name
                                                ).on(
                                                    "keydown.enter",
                                                    lambda result,
    ...
    def init_parameter_clients(self, active_node_list) -> dict:
        ...
        for module in active_node_list:
            list_params_client = self.create_client(
                ListParameters, f"/atos/{module}/list_parameters"
            )
            get_param_value_client = self.create_client(
                GetParameters, f"/atos/{module}/get_parameters"
            )
            set_params_client = self.create_client(
                SetParametersAtomically, f"/atos/{module}/set_parameters_atomically"
            )
            client_list[module] = {
                "list_params_client": list_params_client,
                "get_param_value_client": get_param_value_client,
                "set_params_client": set_params_client,
            }
        return client_list
    ...
    def fetch_parameters_list(self, client_list) -> None:
        ...
        self.get_logger().debug(
            f"Retrieving all parameters in {[node for node in client_list.keys()]}"
        )
        for node in client_list.keys():
            ...
            if not self.service_is_available(list_params_client):
                continue
            threading.Thread(
                target=self.call_service,
                args=(list_params_client, get_param_value_client),
            ).start()
    ...
    def call_service(self, list_params_client, get_param_value_client) -> None:
        ...
        """
        self.get_logger().debug(
            f"Calling service {list_params_client.srv_name}, setting callback service: {get_param_value_client.srv_name}"
        )
        list_params_client.call_async(ListParameters.Request()).add_done_callback(
            lambda future: self.fetch_parameter_values(future, get_param_value_client)
        )
    ...
    def fetch_parameter_values(self, future, client) -> None:
        ...
        client.call_async(get_param_req).add_done_callback(
            lambda future: self.save_params_locally(future, param_names)
        )
    ...
    def save_params_locally(self, future, param_names) -> None:
        ...
        """
        self.get_logger().debug(
            f"Got value(s) {future.result().values} for parameter(s) {param_names}"
        )
        for idx, param_name in enumerate(param_names):
            ...
            match param_type:
                ...
                case ParameterType.PARAMETER_INTEGER:
                    self.parameters[param_name] = (
                        future.result().values[idx].integer_value
                    )
                case ParameterType.PARAMETER_DOUBLE:
                    self.parameters[param_name] = (
                        future.result().values[idx].double_value
                    )
                case ParameterType.PARAMETER_STRING:
                    self.parameters[param_name] = (
                        future.result().values[idx].string_value
                    )
                case ParameterType.PARAMETER_BYTE_ARRAY:
                    self.parameters[param_name] = (
                        future.result().values[idx].byte_array_value
                    )
                case ParameterType.PARAMETER_BOOL_ARRAY:
                    self.parameters[param_name] = (
                        future.result().values[idx].bool_array_value
                    )
                case ParameterType.PARAMETER_INTEGER_ARRAY:
                    self.parameters[param_name] = (
                        future.result().values[idx].integer_array_value
                    )
                case ParameterType.PARAMETER_DOUBLE_ARRAY:
                    self.parameters[param_name] = (
                        future.result().values[idx].double_array_value
                    )
                case ParameterType.PARAMETER_STRING_ARRAY:
                    self.parameters[param_name] = (
                        future.result().values[idx].string_array_value
                    )
                case _:
                    self.get_logger().info(
                        f"Parameter {param_name} has an unsupported type {param_type}"
                    )
    ...
    def set_parameter(self, node_name, param_name, param_value) -> None:
        ...
        """
        self.get_logger().debug(
            f"Setting parameter {param_name} in {node_name} to {param_value}"
        )
        ...
        param_type = self.json_schema["modules"][node_name]["ros__parameters"][
            param_name
        ]["type"]
        self.assign_value_and_type_to_parameter(
            parameter, param_type, param_value
        )  # Checks param_value type and assigns it to the parameter
        param_req = SetParametersAtomically.Request()
        param_req.parameters.append(parameter)

        future = client.call_async(param_req)
        future.add_done_callback(
            lambda future: self.set_param_callback(future, param_name, param_value)
        )

    def assign_value_and_type_to_parameter(
        self, parameter, param_type, param_value
    ) -> None:
        ...
        match param_type:
            ...
            case "byte_array":
                if type(param_value) != list or type(param_value[0]) != int:
                    ui.notify("Please select one or more bytes")
                    return
