...
void systemcontrol_task(TimeType *GPSTime, GSDType *GSD)
{
    ...
    log_message(LOG_LEVEL_INFO,"System control task running with PID: %i",getpid());
    ...
    while(!iExit)
    {
        if(ModeU8 == 0)
        {
            if(ClientSocket <= 0)
            {
                ...
                if(USE_LOCAL_USER_CONTROL == 0)
                {
                    ...
                    log_message(LOG_LEVEL_INFO,"UserControl IP address is %s", inet_ntoa(ip_addr));
            ...
            if (ClientResult <= -1)
            {
                ...
                {
                    log_message(LOG_LEVEL_ERROR,"Failed to receive from command socket");
                    log_message(LOG_LEVEL_ERROR,"Waiting 5 sec before exiting");
                    ...
                    log_message(LOG_LEVEL_ERROR,"System control exiting");
                    exit(1);
                }
            }
        ...
        {   /* use util.c function to call time
            ...
            */
            ...
            if(ServerSocketI32 > 0 && SessionData.SessionIdU32 <= 0)
            {
                ...
                log_message(LOG_LEVEL_INFO,"SessionId: %u", SessionData.SessionIdU32);
                log_message(LOG_LEVEL_INFO,"UserId: %u", SessionData.UserIdU32);
                log_message(LOG_LEVEL_INFO,"Usertype: %d", SessionData.UserTypeU8);
                if(SessionData.SessionIdU32 > 0) { log_message(LOG_LEVEL_INFO,"Sign in success!");} else { log_message(LOG_LEVEL_INFO,"Sign in failed!");}
            }
        ...
        if(server_state == SERVER_STATE_INWORK)
        {
            ...
            {
                log_message(LOG_LEVEL_INFO,"State: %s, OBCState: %s, PreviousCommand: %s", SystemControlStatesArr[server_state], SystemControlOBCStatesArr[OBCStateU8], SystemControlCommandsArr[PreviousSystemControlCommand]);
            ...
            {
                log_message(LOG_LEVEL_WARNING,"Command not allowed, SystemControl is busy in state %s, PreviousCommand: %s", SystemControlStatesArr[server_state], SystemControlCommandsArr[PreviousSystemControlCommand]);
	  ...
	  strcat(data, pcRecvBuffer);
        ...
        case GetServerStatus_0:
            log_message(LOG_LEVEL_INFO,"State: %s, OBCState: %s",SystemControlStatesArr[server_state], SystemControlOBCStatesArr[OBCStateU8]);
            SystemControlCommand = Idle_0;
            bzero(ControlResponseBuffer,SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
            ControlResponseBuffer[0] = server_state;
            ControlResponseBuffer[1] = OBCStateU8;
            log_message(LOG_LEVEL_DEBUG,"GPSMillisecondsU64: %ld", GPSTime->GPSMillisecondsU64); // GPSTime just ticks from 0 up shouldent it be in the global GPStime?
            SystemControlSendControlResponse(SYSTEM_CONTROL_RESPONSE_CODE_OK, "GetServerStatus:", ControlResponseBuffer, 2, &ClientSocket, 0);
            break;
        ...
        case GetServerParameter_1:
            ...
            } else { log_message(LOG_LEVEL_ERROR,"Wrong parameter count in GetServerParameter(Name)!"); SystemControlCommand = Idle_0;}
            break;
        case SetServerParameter_2:
            ...
            } else { log_message(LOG_LEVEL_ERROR,"Wrong parameter count in SetServerParameter(Name, Value)!"); SystemControlCommand = Idle_0;}
            break;
        case CheckFileDirectoryExist_1:
            ...
            } else { log_message(LOG_LEVEL_ERROR,"Wrong parameter count in CheckFFExist(path)!"); SystemControlCommand = Idle_0;}
        break;
        case DeleteFileDirectory_1:
            ...
            } else { log_message(LOG_LEVEL_ERROR,"Wrong parameter count in DeleteFileDirectory(path)!"); SystemControlCommand = Idle_0;}
        break;
        case CreateDirectory_1:
            ...
            } else { log_message(LOG_LEVEL_ERROR,"Wrong parameter count in CreateDirectory(path)!"); SystemControlCommand = Idle_0;}
        break;
        case GetDirectoryContent_1:
            ...
            } else { log_message(LOG_LEVEL_ERROR,"Wrong parameter count in GetDirectoryContent(path)!"); SystemControlCommand = Idle_0;}
        break;
        case DownloadFile_1:
            if(CurrentInputArgCount == CommandArgCount)
            {
        ...
        case UploadFile_3:
            if(CurrentInputArgCount == CommandArgCount)
            {
                ...
                {
                    log_message(LOG_LEVEL_INFO,"Receiving file: %s", SystemControlArgument[0]);
                ...
                { 
                    log_message(LOG_LEVEL_INFO,"Failed receiving file: %s", SystemControlArgument[0]);
            ...
            } else { log_message(LOG_LEVEL_ERROR,"Wrong parameter count in PrepFileRx(path, filesize, packetsize)!"); SystemControlCommand = Idle_0;}
        break;
        ...
        case StartScenario_1:
            if(CurrentInputArgCount == CommandArgCount)
            {
                if(server_state == SERVER_STATE_IDLE && strstr(SystemControlOBCStatesArr[OBCStateU8], "ARMED") != NULL) //Temporary!
                {
                    ...
                    */
                    ...
                    log_message(LOG_LEVEL_INFO,"Current timestamp (gtd): %lu",uiTime );
                    ...
                    log_message(LOG_LEVEL_INFO,"Sending START <%s> (delayed +%s ms)",pcBuffer, SystemControlArgument[1]);
            ...
            } else log_message(LOG_LEVEL_WARNING,"START command parameter count error");
            break;
            /*
        ...
        case Exit_0:
            ...
            if(USE_LOCAL_USER_CONTROL == 0) { close(ServerHandle); ServerHandle = -1;}
            log_message(LOG_LEVEL_INFO,"Server closing");
...
void SystemControlSendMONR(C8* MONRStr, I32* Sockfd, U8 Debug){
  ...
  } else log_message(LOG_LEVEL_ERROR,"MONR string longer than %d bytes!", SYSTEM_CONTROL_SEND_BUFFER_SIZE);
}
...
void SystemControlSendLog(C8* LogString, I32* Sockfd, U8 Debug)
{
    ...
    if(n + 4 < SYSTEM_CONTROL_SEND_BUFFER_SIZE)
    {
        ...
        SystemControlSendBytes(Data, n + 4, Sockfd, 0);
    } else log_message(LOG_LEVEL_ERROR,"Log string longer than %d bytes!", SYSTEM_CONTROL_SEND_BUFFER_SIZE);
...
void SystemControlSendControlResponse(U16 ResponseStatus, C8* ResponseString, C8* ResponseData, I32 ResponseDataLength, I32* Sockfd, U8 Debug)
{
    ...
    if(n + 4 < SYSTEM_CONTROL_SEND_BUFFER_SIZE)
    {
        ...
        SystemControlSendBytes(Data, n + 4, Sockfd, 0);
    } else log_message(LOG_LEVEL_ERROR,"Response data more than %d bytes!", SYSTEM_CONTROL_SEND_BUFFER_SIZE);
}
...
static I32 SystemControlInitServer(int *ClientSocket, int *ServerHandle, struct in_addr *ip_addr)
{
    ...
    log_message(LOG_LEVEL_INFO,"Init control socket");
    ...
    *ServerHandle = socket(AF_INET, SOCK_STREAM, 0);
    ...
    log_message(LOG_LEVEL_INFO,"Listening for connection from client...");
    ...
    while( *ClientSocket = accept(*ServerHandle, (struct sockaddr *) &cli_addr, &cli_length))
    {
        ...
        log_message( LOG_LEVEL_INFO,"Connection accepted!");
        break;

    }

    log_message( LOG_LEVEL_INFO,"Connection received: %s:%i", inet_ntoa(cli_addr.sin_addr), htons(command_server_addr.sin_port));
    ...
    *sockfd = socket(AF_INET, SOCK_STREAM, 0);
    ...
    do
    {
        ...
        if ( iResult < 0)
        {
            ...
            {
                log_message(LOG_LEVEL_WARNING,"Unable to connect to UserControl, retrying in 3 sec...");
                (void)sleep(3);
            }
            else
            {
    ...
    log_message(LOG_LEVEL_DEBUG,"Maestro connected to UserControl: %s:%i",name,port);
    return iResult;
...
static void SystemControlCreateProcessChannel(const C8* name, const U32 port, I32 *sockfd, struct sockaddr_in* addr)
{
    ...
    log_message(LOG_LEVEL_DEBUG,"Creating process channel socket");
    ...
    *sockfd= socket(AF_INET, SOCK_DGRAM, 0);
    ...
    log_message(LOG_LEVEL_INFO,"Created process channel socket and address: %s:%d",name,port);
}
    ...
    if(fd > 0)
    {
        ...
        for(i = 0; i < RowCount; i++)
        {
            ...
            if(ptr1 != NULL && (U64)ptr2 >= (U64)ptr1 && ParameterFound == 0)
            {
                ...
                if(Debug)
                {
                    log_message(LOG_LEVEL_DEBUG,"Changed parameter: %s", NewRow);
                }
...
I32 SystemControlReadServerParameter(C8 *ParameterName, C8 *ReturnValue, U8 Debug)
{
    ...
    if(Debug)
    {
        log_message(LOG_LEVEL_DEBUG,"%s = %s\n", ParameterName, ReturnValue);
    }
...
I32 SystemControlReadServerParameterList(C8 *ParameterList, U8 Debug)
{
    ...
    if(Debug)
    {
        log_message(LOG_LEVEL_DEBUG,"ParameterList = %s\n", ParameterList);
    }
...
I32 SystemControlBuildFileContentInfo(C8 *Path, C8 *ReturnValue, U8 Debug)
{
    ...
    *(ReturnValue+3) = (U8)st.st_size;
    ...
    if(Debug) log_message(LOG_LEVEL_DEBUG,"Filesize %d of %s", (I32)st.st_size, CompletePath);
...
I32 SystemControlCheckFileDirectoryExist(C8 *ParameterName, C8 *ReturnValue, U8 Debug)
{
    ...
    *ReturnValue = PATH_INVALID_MISSING; 
    ...
    if(Debug) log_message(LOG_LEVEL_DEBUG,"%d %s", *ReturnValue, CompletePath);
...
I32 SystemControlDeleteFileDirectory(C8 *Path, C8 *ReturnValue, U8 Debug)
{
    ...
    *ReturnValue = PATH_INVALID_MISSING; 
    ...
    if(*ReturnValue == SUCCEDED_DELETE) log_message(LOG_LEVEL_INFO,"Deleted %s", CompletePath);
    else if(*ReturnValue == FAILED_DELETE) log_message(LOG_LEVEL_INFO,"Failed to delete %s", CompletePath);
...
I32 SystemControlCreateDirectory(C8 *Path, C8 *ReturnValue, U8 Debug)
{
    ...
    *ReturnValue = PATH_INVALID_MISSING; 
    ...
    if(*ReturnValue == SUCCEDED_CREATE_FOLDER) log_message(LOG_LEVEL_INFO,"Directory created: %s", CompletePath);
...
I32 SystemControlUploadFile(C8 *Path, C8 *FileSize, C8 *PacketSize, C8 *ReturnValue, U8 Debug)
{
    ...
    if(Debug)
    {
        log_message(LOG_LEVEL_DEBUG,"Upload file:");
        log_message(LOG_LEVEL_DEBUG,"%s", Path);
        log_message(LOG_LEVEL_DEBUG,"%s", FileSize);
        log_message(LOG_LEVEL_DEBUG,"%s", PacketSize);
        log_message(LOG_LEVEL_DEBUG,"%s", CompletePath);
    }
...
I32 SystemControlReceiveRxData(I32 *sockfd, C8 *Path, C8 *FileSize, C8 *PacketSize, C8 *ReturnValue, U8 Debug)
{
    ...
    if(Debug)
    {
        log_message(LOG_LEVEL_DEBUG,"Receive Rx data:");
        log_message(LOG_LEVEL_DEBUG,"%s", Path);
        log_message(LOG_LEVEL_DEBUG,"%s", FileSize);
        log_message(LOG_LEVEL_DEBUG,"%s", PacketSize);
        log_message(LOG_LEVEL_DEBUG,"%s", CompletePath);
    ...
    if(fd != NULL)
    {
        ...
        log_message(LOG_LEVEL_DEBUG,"Rec count = %d, Req count = %d", TotalRxCount, FileSizeU32);
...
I32 SystemControlSendFileContent(I32 *sockfd, C8 *Path, C8 *PacketSize, C8 *ReturnValue, U8 Remove, U8 Debug)
{
    ...
    if(Debug)
    {
        log_message(LOG_LEVEL_DEBUG,"Send file content:");
        log_message(LOG_LEVEL_DEBUG,"%s", Path);
        //printf("%s\n", FileSize);
        log_message(LOG_LEVEL_DEBUG,"%s", PacketSize);
        log_message(LOG_LEVEL_DEBUG,"%s", CompletePath);
    ...
    if(fd != NULL)
    {
        ...
        log_message(LOG_LEVEL_INFO,"Sent file: %s, total size = %d, transmissions = %d", CompletePath, (PacketSizeU16*TransmissionCount+RestCount), i + 1);
