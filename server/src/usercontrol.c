...
/*------------------------------------------------------------
  ...
  ------------------------------------------------------------*/
UserControlCommand_t UserControlFindCommand(const char *CommandBuffer, UserControlCommand_t * CurrentCommand,
											int *ArgCount);
...
int main(int argc, char *argv[]) {
	...
	unsigned char TestBuffer[] =
		{ 7, 200, 65, 36, 135, 231, 3, 111, 230, 191, 0, 190, 10, 254, 0, 0, 86, 195, 44, 236, 1, 104, 0, 6 };

	char MessageBuffer[100];
	int MessageLength, j;
	int RowCount, i, Rest;
	char TrajBuffer[COMMAND_DOTM_ROWS_IN_TRANSMISSION * COMMAND_DOTM_ROW_MESSAGE_LENGTH +
	...
	if (socketfd >= 0) {
		...
		while (!iExit) {
			...
			if (strcmp(pcBuffer, "cc") != 0 &&
				...
				strcmp(pcBuffer, "cb") != 0 && strcmp(pcBuffer, "sx") != 0 && strcmp(pcBuffer, "sc") != 0) {
			...
			case tt_0:
				...
				uiTime =
					(uint64_t) tTime.tv_sec * 1000L + (uint64_t) tTime.tv_nsec / 1000000L -
					(uint64_t) MS_FROM_1970_TO_2004_NO_LEAP_SECS +
					(uint64_t) DIFF_LEAP_SECONDS_UTC_ETSI *1000;
				printf("Time: %lx\n", uiTime);
				printf("Time: %ld\n", uiTime);
			...
			case tp_0:
				...
				printf
					("(57.7773716086, 12.7804629583 ,57.7773716000, 12.7804629000) => Calc d = %4.4f m, iterations = %d\n",
			...
			case tsp_1:
				...
				if (CurrentCommandArgCount == CommandArgCount) {
					...
					//2.599, 57.7773600, 12.7804720 ,57.7776768, 12.7808503
					//2.577, 57.7773600, 12.7804720 ,57.7776776, 12.7808498
					//2.551, 57.7773600, 12.7804720 ,57.7776784, 12.7808493
					//5.602, 57.7773600, 12.7804720 ,57.7775458, 12.7809363
					//35.220, 57.7773600, 12.7804720 ,57.7778226, 12.7814385
					//25.505, 57.7773600, 12.7804720 ,57.7774398, 12.7817521
					//21.389, 57.7773600, 12.7804720 ,57.7772277, 12.7819276
					//19.2, 57.7773600, 12.7804720 ,57.7771761, 12.7817484
					//19.464, 57.7773600, 12.7804720 ,57.7771804, 12.7817730
					//19.273, 57.7773600, 12.7804720 ,57.7771772, 12.7817548
					...
					if (OP.OrigoDistance > -1) {
						...
						printf("Distance calculation to origo failed to converge.\n");
					UserControlResetInputVariables();
				}
				else
					CurrentCommandArgCount++;
			...
			case tdopm_0:
				...
				do {
					...
					printf("Transmission %d: %d bytes left to send.\n", ++i,
						   Rest * COMMAND_DOTM_ROW_MESSAGE_LENGTH);
		...
		printf("Failed to start UserControl client!\n");
}
...
UserControlCommand_t UserControlFindCommand(const char *CommandBuffer, UserControlCommand_t * CurrentCommand,
	...
	for (command = idle_0; command != nocommand; command++) {
		...
		strncpy(UserControlStrippedCommand, UserControlCommandsArr[(int)command],
				(uint64_t) strchr(UserControlCommandsArr[(int)command],
								  '_') - (uint64_t) UserControlCommandsArr[(int)command]);
		strncpy(UserControlCommandArgCnt, strchr(UserControlCommandsArr[(int)command], '_') + 1,
				strlen(UserControlCommandsArr[(int)command]) -
				((uint64_t) strchr(UserControlCommandsArr[(int)command], '_') -
				 (uint64_t) UserControlCommandsArr[(int)command] + 1));
