...
MonitorDisplay::MonitorDisplay() :
  ...
  pose_valid_(false) {
	...
	shaft_radius_property_ = new rviz_common::properties::FloatProperty(
	  "Shaft Radius", 0.05f, "Radius of the arrow's shaft, in meters.", this, SLOT(updateArrowGeometry()));

	head_length_property_ = new rviz_common::properties::FloatProperty(
	  "Head Length", 0.3f, "Length of the arrow's head, in meters.", this, SLOT(updateArrowGeometry()));

	head_radius_property_ = new rviz_common::properties::FloatProperty(
	  "Head Radius", 0.1f, "Radius of the arrow's head, in meters.", this, SLOT(updateArrowGeometry()));
...
void MonitorDisplay::processMessage(atos_interfaces::msg::Monitor::ConstSharedPtr msg) {
	...
	if (!context_->getFrameManager()->transform(msg->pose.header, msg->pose.pose, position, orientation)) {
...
void MonitorDisplay::onInitialize() {
	...
	arrow_ = std::make_unique<rviz_rendering::Arrow>(scene_manager_,
													 scene_node_,
