...
ScenarioExecution::ScenarioExecution() :
  IntegrationTesting(testName) {
	RCLCPP_INFO(get_logger(), "Started integration test: %s", testName.c_str());
	getObjectTrajectoryClient =
	  this->create_client<atos_interfaces::srv::GetObjectTrajectory>(atosNamespace + ServiceNames::getObjectTrajectory);
	runIntegrationTest();
}
...
void ScenarioExecution::checkObjectStoppedAtLastPoint() {
	...
	while (isObjectMoving) {
		...
		monitorSub = this->create_subscription<atos_interfaces::msg::Monitor>(
		  "/atos/object_1/object_monitor",
		  10,
		  std::bind(&ScenarioExecution::placeholderCallback, this, std::placeholders::_1));
	...
	if (abs(distanceX) < distanceThreshold && abs(distanceY) < distanceThreshold) {
		...
		RCLCPP_ERROR(
		  get_logger(),
		  "Object did not follow trajectory succesfully. Distance from end point of trajectory is x: %f, y: %f",
		  distanceX,
		  distanceY);
	}
}
...
void ScenarioExecution::printResult() {
	...
	ss << std::left << std::setfill(' ') << std::setw(width) << "State" << std::setw(width) << "Expected state"
