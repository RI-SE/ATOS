...
TruckObjectControl::TruckObjectControl() : Node("truck_object_control") {
  ...
  RCLCPP_INFO(get_logger(),
              ...
              cot_tls_enabled_ ? "tls" : "tcp", cot_tcp_bind_address_.c_str(), cot_tcp_port_);
}
...
void TruckObjectControl::acceptTcpClients() {
  while (tcp_running_.load()) {
    ...
    if (::setsockopt(client_fd, IPPROTO_TCP, TCP_NODELAY, &tcp_no_delay, sizeof(tcp_no_delay)) < 0) {
      RCLCPP_WARN(get_logger(),
                  "Failed to enable TCP_NODELAY for client socket (errno=%d). Commands may be delayed.", errno);
    }
      ...
      if (cot_tls_enabled_) {
        ...
        if (poll_result < 0) {
          ...
          RCLCPP_WARN(get_logger(),
                      ...
                      peer_name.c_str(), errno, std::strerror(errno));
          break;
        }
        if ((pfd.revents & (POLLERR | POLLHUP | POLLNVAL)) != 0) {
          RCLCPP_INFO(get_logger(), "TLS client %s socket closed or errored (revents=0x%x).",
                      peer_name.c_str(), pfd.revents);
          break;
        }
        ...
        if (received <= 0) {
          ...
          RCLCPP_WARN(get_logger(),
                      ...
                      peer_name.c_str(), describeTlsError(ssl_read_error).c_str());
          break;
        }
      } else {
        ...
        }
        {
          ...
          if (existing != uid_to_client_fd_.end() && existing->second != client_fd) {
            RCLCPP_WARN(get_logger(),
                        ...
                        observation.truck_id.c_str(), existing->second, client_fd, peer_name.c_str());
          }
...
void TruckObjectControl::evaluateAndPublishSpeedCommand() {
    ...
    for (const auto &[id, state] : trucks_) {
      if (!state.tcp_connected) {
        RCLCPP_WARN_THROTTLE(
            ...
            id.c_str(),
            state.path_name.c_str(), state.distance_along_trajectory_m);
        continue;
      }
      connected.push_back(ConnectedTruck{id, state.path_name, state.distance_along_trajectory_m, state.speed_mps,
  ...
  if (connected.empty()) {
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 3000,
                         "No TCP-connected trucks available for command evaluation.");
    return;
  }
  ...
  command.data = "target_speed_mps=" + fleet_target_speed_mps_value +
                 ";scope=all_connected_with_valid_tcp" + ";reason=" + fleet_reason +
...
void TruckObjectControl::updateTruckTcpStatus(const std::string &target_id, const std::string &command,
...
void TruckObjectControl::clientSenderLoop(const std::shared_ptr<TcpClientSession> &session) {
  while (tcp_running_.load()) {
    ...
    if (cot_tls_enabled_) {
      ...
      RCLCPP_INFO(get_logger(), "Sent TLS speed command to uid=%s fd=%d: %s", target_id.c_str(), session->fd,
                  command.c_str());
    ...
    RCLCPP_INFO(get_logger(), "Sent TCP speed command to uid=%s fd=%d: %s", target_id.c_str(), session->fd,
                command.c_str());
    updateTruckTcpStatus(target_id, command, "");
  }
}
...
void TruckObjectControl::disconnectClientSession(const std::shared_ptr<TcpClientSession> &session,
  ...
  RCLCPP_INFO(get_logger(), "TruckObject TCP client disconnected: %s (%s)", session->peer_name.c_str(),
              reason.c_str());
}
