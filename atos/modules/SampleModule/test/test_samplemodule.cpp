...
TEST_F(SampleModuleTest, testPublishesOnInitResponseWhenInitIsPublished) {
	...
	auto smOnInitResponseCallback =
	  [&receivedMsg, &subCalled](const ROSChannels::SampleModuleTestForInitResponse::message_type::SharedPtr) {
		  receivedMsg = true;
		  subCalled.set_value();
	  };

	std::string topicname =
	  std::string(sampleModule->get_namespace()) + ROSChannels::SampleModuleTestForInitResponse::topicName;
...
TEST_F(SampleModuleTest, testThatServiceResponseIsSetToSuccessWhenServiceIsCalled) {
	...
	auto testServiceCalledCallback =
	  [&receivedReq, &serviceCalled](const rclcpp::Client<std_srvs::srv::SetBool>::SharedFuture response) {
