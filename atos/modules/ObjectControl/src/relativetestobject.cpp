 ...
 * \return Transformed monitor data
 */
ObjectMonitorType RelativeTestObject::transformCoordinate(const ObjectMonitorType& point,
	...
	if (steady_clock::now() > nextPrintTime) {
		print		  = debug;
		nextPrintTime = steady_clock::now() - nextPrintTime > printInterval ? steady_clock::now() + printInterval
	...
	retval.position.isPositionValid =
	  anchor.position.isPositionValid && anchor.position.isHeadingValid && point.position.isPositionValid;
	...
	retval.speed.isLateralValid	   = retval.speed.isLongitudinalValid =
	  anchor.speed.isLateralValid && anchor.speed.isLongitudinalValid && point.speed.isLateralValid &&
	  point.speed.isLongitudinalValid;
