...
ObjectControl::ObjectControl(std::shared_ptr<rclcpp::executors::MultiThreadedExecutor> exec) :
  ...
  stateChangePub(*this) {
	...
	stateService = create_service<atos_interfaces::srv::GetObjectControlState>(
	  ServiceNames::getObjectControlState, std::bind(&ObjectControl::onRequestState, this, _1, _2));
...
void ObjectControl::onRequestState(const std::shared_ptr<atos_interfaces::srv::GetObjectControlState::Request>,
...
void ObjectControl::handleActionConfigurationCommand(const TestScenarioCommandAction& /* action */) {
	// TODO: Implement
}

void ObjectControl::handleExecuteActionCommand(const uint16_t& actionID,
											   const std::chrono::system_clock::time_point& when) {
	auto delayedExecutor = [&]() {
		using namespace std::chrono;
		RCLCPP_DEBUG(get_logger(),
					 "Executing action %u in %ld ms",
					 actionID,
...
void ObjectControl::onStartObjectMessage(const StartObject::message_type::SharedPtr strtObj) {
	sm.process_event(state_machine::events::StartObject{
	  strtObj->id,
	  std::chrono::system_clock::time_point{std::chrono::seconds{strtObj->stamp.sec} +
											std::chrono::nanoseconds{strtObj->stamp.nanosec}}});
}
	...
	auto idsCallback = [&](const rclcpp::Client<atos_interfaces::srv::GetObjectIds>::SharedFuture future) {
		...
		for (const auto id : idResponse->ids) {
			...
			auto ipCallback = [&](const rclcpp::Client<atos_interfaces::srv::GetObjectIp>::SharedFuture future) {
				...
				if (status != 0) {
					RCLCPP_ERROR(
					  get_logger(), "Failed to resolve address for object ID %u: %s", id, gai_strerror(status));
					ipLoaded.set_value(false);
					return;
				}
			...
			auto originCallback = [&](const rclcpp::Client<atos_interfaces::srv::GetTestOrigin>::SharedFuture future) {
				...
				objects.at(id)->setOrigin({origin->origin.position.latitude,
				...
				RCLCPP_INFO(get_logger(),
							"Got origin for object ID %u: (%.6f, %.6f, %.3f)",
							id,
...
void ObjectControl::transformScenarioRelativeTo(const uint32_t objectID) {
...
void ObjectControl::beginConnectionAttempt() {
	...
	for (const auto id : getVehicleIDs()) {
		auto t =
		  std::thread(&ObjectControl::connectToObject, this, std::ref(objects.at(id)), std::ref(connStopReqFuture));
		t.detach();
	}
}
...
void ObjectControl::disconnectObject(const uint32_t id) {
	objects.at(id)->disconnect();
...
void ObjectControl::uploadObjectConfiguration(const uint32_t id) {
			...
			if (objects.at(id)->isConnected()) {
				...
				if (diff > objects.at(id)->getMaxAllowedMonitorPeriod()) {
					RCLCPP_WARN(get_logger(),
								"MONR timeout for object %u: %ld ms > %ld ms",
								id,
								diff.count(),
								objects.at(id)->getMaxAllowedMonitorPeriod().count());
					objects.at(id)->disconnect();
...
OsiHandler::LocalObjectGroundTruth_t ObjectControl::buildOSILocalGroundTruth(const MonitorMessage& monr) const {
	...
	try {
		if (!obj->isConnected()) {
			try {
				...
				RCLCPP_ERROR(
				  get_logger(), "Connection attempt for object %u failed: %s", obj->getTransmitterID(), e.what());
				obj->disconnect();
				return;
			...
			try {
				...
				while (true) {
					...
					try {
						...
						if (connectionHeartbeats-- > 0) {
							...
							throw std::runtime_error("No monitor reply after " + std::to_string(maxConnHeabs) +
													 " heartbeats. Details:\n" + e.what());
						}
					}
					switch (objState) {
						...
						case OBJECT_STATE_INIT:
							if (initializingMonrs-- > 0) {
								...
								RCLCPP_INFO(get_logger(),
											"Connected object %u in initializing state after connection",
											obj->getTransmitterID());
								sm.process_event(state_machine::events::Abort{});
							}
							break;
						...
						default:
							RCLCPP_INFO(
							  get_logger(), "Connected to object %u in unknown state", obj->getTransmitterID());
							sm.process_event(state_machine::events::Abort{});
					}
					break;
				}
				...
				RCLCPP_ERROR(get_logger(),
							 "Connection startup procedure failed for object %u: %s",
							 obj->getTransmitterID(),
							 e.what());
				obj->disconnect();
				// TODO: connection failed event?
		...
		RCLCPP_ERROR(get_logger(), "Bad connection attempt for object %u: %s", obj->getTransmitterID(), e.what());
		obj->disconnect();
		// TODO: connection failed event?
	}
};
...
/**
 * @brief Callback function for the trajectory service call. Sets the new trajectory for the object in ATOS and sends it
 * to the object
 *
...
/**
 * @brief Callback function for the return trajectory service call. Sets the new trajectory for the object in ATOS and
 * sends it to the object
 *
 * @param future the future object containing the response from the return trajectory service call
 */
void ObjectControl::returnTrajectoryCallback(
...
/**
 * @brief Sets the trajectory for a given object by requesting it from the esminiAdapter or BackToStart module depending
 * on the current state (resetting)
 *
 * @param id the object id
 */
void ObjectControl::setObjectTrajectory(uint32_t id) {
	...
	if (this->isResetting) {
		using namespace std::chrono_literals;
		if (!this->returnTrajectoryClient->wait_for_service(1s)) // TODO: This is blocking any other callbacks for 1s if
																 // the service is not available. Consider new thread.
		{
		...
		// Send the request to the return trajectory service asynchronously and bind the callback function for when the
		// response is received
		returnTrajectoryClient->async_send_request(returnTrajectoryRequest,
												   std::bind(&ObjectControl::returnTrajectoryCallback, this, _1));
	} else {
...
void ObjectControl::startObject(uint32_t id, std::chrono::system_clock::time_point startTime) {
	try {
		...
		str.pop_back();
		RCLCPP_WARN(
		  get_logger(), "Attempted to start nonexistent object %u - configured objects are%s", id, str.c_str());
	}
}

auto readyOrDisconnected = [](std::shared_ptr<TestObject> obj) {
	return obj->getState() == OBJECT_STATE_DISARMED || obj->getState() == OBJECT_STATE_ARMED || !obj->isConnected();
};
...
bool ObjectControl::areAllObjects(std::function<bool(const std::shared_ptr<TestObject>)> predicate) const {
	return std::all_of(
	  objects.cbegin(), objects.cend(), [predicate](const std::pair<const uint32_t, std::shared_ptr<TestObject>>& obj) {
...
bool ObjectControl::isAnyObjectIn(const ObjectStateType state) {
	return std::any_of(
	  objects.cbegin(), objects.cend(), [state](const std::pair<const uint32_t, std::shared_ptr<TestObject>>& obj) {
		  return obj.second->getState() == state;
	  });
}

bool ObjectControl::areAllObjectsIn(const ObjectStateType state) {
	return std::all_of(
	  objects.cbegin(), objects.cend(), [state](const std::pair<const uint32_t, std::shared_ptr<TestObject>>& obj) {
		  return obj.second->getState() == state;
	  });
...
bool ObjectControl::isAnyObjectIn(const std::set<ObjectStateType>& states) {
	return std::any_of(
	  objects.cbegin(), objects.cend(), [states](const std::pair<const uint32_t, std::shared_ptr<TestObject>>& obj) {
		  return states.find(obj.second->getState()) != states.end();
	  });
}

bool ObjectControl::areAllObjectsIn(const std::set<ObjectStateType>& states) {
	return std::all_of(
	  objects.cbegin(), objects.cend(), [states](const std::pair<const uint32_t, std::shared_ptr<TestObject>>& obj) {
		  return states.find(obj.second->getState()) != states.end();
	  });
...
void ObjectControl::startControlSignalSubscriber() {
	controlSignalSub =
	  std::make_shared<ControlSignal::Sub>(*this, std::bind(&ObjectControl::onControlSignalMessage, this, _1));
}
...
void ObjectControl::publishScenarioInfoToJournal() {
	...
	for (const auto& object : objects) {
		...
		ss << "\n> Object " << index << ": \n"
		   << "\t - ID: " << object.first << "\n"
		   << "\t - IP: " << ip_str << "\n"
		   << "\t - Origin: (" << testObject->getOrigin().latitude_deg << ", " << testObject->getOrigin().longitude_deg
		   << ", " << testObject->getOrigin().altitude_m << ")\n"
