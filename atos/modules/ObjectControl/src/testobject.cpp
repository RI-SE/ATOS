...
void TestObject::setObjectIP(const in_addr_t newIP) {
...
void TestObject::setCommandAddress(const sockaddr_in& newAddr) {
	if (!this->comms.isConnected()) {
		this->comms.cmd.addr = newAddr;
	}
}

void TestObject::setMonitorAddress(const sockaddr_in& newAddr) {
	if (!this->comms.isConnected()) {
		this->comms.mntr.addr = newAddr;
	}
}

void TestObject::setOsiAddress(const sockaddr_in& newAddr) {
	if (!this->comms.isConnected()) {
...
void TestObject::setTriggerStart(const bool startOnTrigger) {
	auto st = this->getState();
	if (st != OBJECT_STATE_ARMED && st != OBJECT_STATE_RUNNING && st != OBJECT_STATE_POSTRUN &&
		st != OBJECT_STATE_ABORTING && st != OBJECT_STATE_REMOTE_CONTROL && st != OBJECT_STATE_PRE_ARMING &&
...
ObjectStateType TestObject::getState(const bool awaitUpdate, const std::chrono::milliseconds timeout) {
	if (awaitUpdate) {
...
void TestObject::parseConfigurationFile(const fs::path& objectFile) {
	struct sockaddr_in addr;
...
void TestObject::sendHeartbeat(const ControlCenterStatusType ccStatus) {
...
void TestObject::sendOsiData(const OsiHandler::LocalObjectGroundTruth_t& osidata,
...
void TestObject::sendStart(std::chrono::system_clock::time_point startTime) {
	...
	strt.startTime.tv_usec =
	  std::chrono::duration_cast<std::chrono::microseconds>(startTime.time_since_epoch()).count() % 1000000;
