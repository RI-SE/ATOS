...
Channel& operator<<(Channel& chnl, const HeabMessageDataType& heartbeat) {
	...
	auto nBytes = encodeHEABMessage(chnl.populateHeaderType(&header),
									&heartbeat.dataTimestamp,
									heartbeat.controlCenterStatus,
									chnl.transmitBuffer.data(),
									chnl.transmitBuffer.size(),
									false);
	if (nBytes < 0) {
...
Channel& operator<<(Channel& chnl, const ObjectSettingsType& settings) {
	MessageHeaderType header;
	auto nBytes = encodeOSEMMessage(
	  chnl.populateHeaderType(&header), &settings, chnl.transmitBuffer.data(), chnl.transmitBuffer.size(), false);
	if (nBytes < 0) {
...
Channel& operator<<(Channel& chnl, const ATOS::Trajectory& traj) {
	...
	nBytes = encodeTRAJMessageHeader(chnl.populateHeaderType(&header),
									 ...
									 traj.name.c_str(),
									 traj.name.length(),
									 static_cast<uint32_t>(traj.points.size()),
									 chnl.transmitBuffer.data(),
									 chnl.transmitBuffer.size(),
									 false);
	if (nBytes < 0) {
	...
	for (const auto& pt : traj.points) {
		...
		nBytes = encodeTRAJMessagePoint(&relTime,
										pos,
										spd,
										acc,
										static_cast<float>(pt.getCurvature()),
										chnl.transmitBuffer.data(),
										chnl.transmitBuffer.size(),
										false);
		if (nBytes < 0) {
...
Channel& operator<<(Channel& chnl, const ObjectCommandType& cmd) {
	MessageHeaderType header;
	auto nBytes = encodeOSTMMessage(
	  chnl.populateHeaderType(&header), cmd, chnl.transmitBuffer.data(), chnl.transmitBuffer.size(), false);
	if (nBytes < 0) {
...
Channel& operator<<(Channel& chnl, const StartMessageType& strt) {
	MessageHeaderType header;
	auto nBytes = encodeSTRTMessage(
	  chnl.populateHeaderType(&header), &strt, chnl.transmitBuffer.data(), chnl.transmitBuffer.size(), false);
	if (nBytes < 0) {
...
Channel& operator<<(Channel& chnl, const ControlSignal::message_type::SharedPtr csp) {
	...
	MessageHeaderType header;
	auto nBytes = encodeRCMMMessage(
	  chnl.populateHeaderType(&header), &rcmm, chnl.transmitBuffer.data(), chnl.transmitBuffer.size(), false);
	if (nBytes < 0) {
...
ISOMessageID Channel::pendingMessageType(bool awaitNext) {
	auto result = recv(
	  this->socket, this->receiveBuffer.data(), this->receiveBuffer.size(), (awaitNext ? 0 : MSG_DONTWAIT) | MSG_PEEK);
	if (result < 0 && !awaitNext && (errno == EAGAIN || errno == EWOULDBLOCK)) {
		return MESSAGE_ID_INVALID;
	} else if (result < 0) {
		throw std::runtime_error(std::string("Failed to check pending message type (recv: ") + strerror(errno) + ")");
	} else if (result == 0) {
...
void Channel::connect(std::shared_future<void> stopRequest, const std::chrono::milliseconds retryPeriod) {
	char ipString[INET_ADDRSTRLEN];
	std::stringstream errMsg;

	if (inet_ntop(AF_INET, &this->addr.sin_addr, ipString, sizeof(ipString)) == nullptr) {
	...
	RCLCPP_INFO(get_logger(), "Attempting %s connection to %s:%u", type.c_str(), ipString, ntohs(this->addr.sin_port));

	while (true) {
		if (::connect(this->socket, reinterpret_cast<struct sockaddr*>(&this->addr), sizeof(this->addr)) == 0) {
			break;
		} else {
			RCLCPP_ERROR(get_logger(),
						 "Failed %s connection attempt to %s:%u, retrying in %.3f s ...",
						 type.c_str(),
						 ipString,
						 ntohs(this->addr.sin_port),
						 retryPeriod.count() / 1000.0);
			if (stopRequest.wait_for(retryPeriod) != std::future_status::timeout) {
