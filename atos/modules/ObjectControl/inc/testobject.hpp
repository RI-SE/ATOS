...
public:
	...
	virtual uint32_t getTransmitterID() const {
		return conf.getTransmitterID();
	}
	virtual std::string getTrajectoryFileName() const {
		return conf.getTrajectoryFileName();
	}
	virtual ATOS::Trajectory getTrajectory() const {
		return conf.getTrajectory();
	}
	virtual GeographicPositionType getOrigin() const {
		return conf.getOrigin();
	}
	...
	virtual ObjectStateType getState() const {
		return isConnected() ? state : OBJECT_STATE_UNKNOWN;
	}
	virtual ObjectMonitorType getLastMonitorData() const {
		return lastMonitor;
	}
	virtual ObjectConfig getObjectConfig() const {
		return conf;
	}
	virtual void setTrajectory(const ATOS::Trajectory& newTrajectory) {
		conf.setTrajectory(newTrajectory);
	}
	virtual void setTransmitterID(const uint32_t newID) {
		conf.setTransmitterID(newID);
	}
	...
	virtual void interruptSocket() {
		comms.interruptSocket();
	}

	virtual bool isAnchor() const {
		return conf.isAnchor();
	}
	virtual bool isOsiCompatible() const {
		return conf.isOSI();
	}
	virtual bool isStartingOnTrigger() const {
		return startOnTrigger;
	}
	virtual std::string toString() const;
	virtual std::string getProjString() const {
		return conf.getProjString();
	}
	...
	virtual void disconnect() {
		RCLCPP_INFO(get_logger(), "Disconnecting object %u", this->getTransmitterID());
	...
	virtual std::chrono::milliseconds getTimeSinceLastMonitor() const {
		...
		return std::chrono::duration_cast<std::chrono::milliseconds>(clock::now() - lastMonitorTime);
	}
...
protected:
	...
	std::chrono::milliseconds maxAllowedMonitorPeriod =
	  std::chrono::milliseconds(static_cast<unsigned int>(1000.0 * 100.0 / MONR_EXPECTED_FREQUENCY_HZ));
};

// Template specialisation of std::less for TestObject
namespace std {
template<>
struct less<TestObject> {
