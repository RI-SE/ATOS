...
MqttBridge::MqttBridge() :
  ...
  customCommandActionMsgSub(*this, std::bind(&MqttBridge::onCustomCommandActionMsg, this, _1)),
...
void MqttBridge::loadParameters() {
	...
	const auto mqtt2ros_mqtt_topics =
	  declare_parameter<std::vector<std::string>>("mqtt2ros.mqtt_topics", std::vector<std::string>(), param_desc);
	for (const auto& mqtt_topic : mqtt2ros_mqtt_topics) {
		...
		declare_parameter(fmt::format("mqtt2ros.{}.advanced.ros.queue_size", mqtt_topic),
						  rclcpp::ParameterType::PARAMETER_INTEGER,
						  param_desc);
	}
	...
	const auto ros2mqtt_ros_topics =
	  declare_parameter<std::vector<std::string>>("ros2mqtt.ros_topics", std::vector<std::string>(), param_desc);
	for (const auto& ros_topic : ros2mqtt_ros_topics) {
		param_desc.description = "MQTT topic on which the corresponding ROS "
								 "messages are sent to the broker";
		declare_parameter(
		  fmt::format("ros2mqtt.{}.mqtt_topic", ros_topic), rclcpp::ParameterType::PARAMETER_STRING, param_desc);
		param_desc.description = "ROS subscriber queue size";
		declare_parameter(fmt::format("ros2mqtt.{}.advanced.ros.queue_size", ros_topic),
						  rclcpp::ParameterType::PARAMETER_INTEGER,
						  param_desc);
		param_desc.description = "MQTT QoS value";
	...
	for (const auto& mqtt_topic : mqtt2ros_mqtt_topics) {
		...
		if (get_parameter(fmt::format("mqtt2ros.{}.ros_topic", mqtt_topic), ros_topic_param)) {
			...
			if (get_parameter(fmt::format("mqtt2ros.{}.advanced.ros.queue_size", mqtt_topic), queue_size_param)) {
	...
	for (const auto& ros_topic : ros2mqtt_ros_topics) {
		...
		if (get_parameter(fmt::format("ros2mqtt.{}.mqtt_topic", ros_topic), mqtt_topic_param)) {
			...
			rclcpp::Parameter queue_size_param;
			if (get_parameter(fmt::format("ros2mqtt.{}.advanced.ros.queue_size", ros_topic), queue_size_param))
				ros2mqtt.ros.queue_size = queue_size_param.as_int();

			// ros2mqtt[k]/advanced/mqtt/qos
			rclcpp::Parameter qos_param;
			if (get_parameter(fmt::format("ros2mqtt.{}.advanced.mqtt.qos", ros_topic), qos_param))
...
void MqttBridge::setupMqtt2RosBridge() {
	new_mqtt2ros_bridge_service_ = create_service<atos_interfaces::srv::NewMqtt2RosBridge>(
	  "~/new_mqtt2ros_bridge",
	  std::bind(&MqttBridge::newMqtt2RosBridge, this, std::placeholders::_1, std::placeholders::_2));

	// Create a request and response for each mqtt2ros bridge and put into a
	// map
	auto serviceCallMap = std::map<std::shared_ptr<atos_interfaces::srv::NewMqtt2RosBridge::Response>,
	...
	while (std::any_of(
	  serviceCallMap.begin(), serviceCallMap.end(), [](const auto& pair) { return !pair.first->success; })) {
		// Get all unsuccesful responses
		auto failedResponses = std::vector<std::shared_ptr<atos_interfaces::srv::NewMqtt2RosBridge::Response>>();
...
void MqttBridge::setupRos2MqttBridge() {
	new_ros2mqtt_bridge_service_ = create_service<atos_interfaces::srv::NewRos2MqttBridge>(
	  "~/new_ros2mqtt_bridge",
	  std::bind(&MqttBridge::newRos2MqttBridge, this, std::placeholders::_1, std::placeholders::_2));
	...
	for (auto& [ros_topic, ros2mqtt] : ros2mqtt_) {
		if (all_topics_and_types.count(ros_topic)) {
			...
			std::function<void(const std::shared_ptr<rclcpp::SerializedMessage> msg)> bound_callback_func =
			  std::bind(&MqttBridge::ros2mqtt, this, std::placeholders::_1, ros_topic);
			try {
				ros2mqtt.ros.subscriber =
				  create_generic_subscription(ros_topic, msg_type, ros2mqtt.ros.queue_size, bound_callback_func);
			} catch (rclcpp::exceptions::RCLError& e) {
				RCLCPP_ERROR(get_logger(), "Failed to create generic subscriber: %s", e.what());
				return;
			}
			RCLCPP_INFO(get_logger(), "Subscribed to ROS topic '%s' of type '%s'", ros_topic.c_str(), msg_type.c_str());
		}
	}
}
...
void MqttBridge::ros2mqtt(const std::shared_ptr<rclcpp::SerializedMessage>& serialized_msg,
	...
	RCLCPP_INFO(get_logger(), "Received ROS message on topic '%s'", ros_topic.c_str());

	try {
		mqtt::message_ptr mqtt_msg =
		  mqtt::make_message(mqtt_topic, payload_buffer.data(), payload_buffer.size(), ros2mqtt.mqtt.qos, true);
		client_->publish(mqtt_msg);
		RCLCPP_INFO(get_logger(), "Published ROS message to MQTT topic '%s'", mqtt_topic.c_str());
...
void MqttBridge::setupClient() {
	...
	try {
		client_ = std::shared_ptr<mqtt::async_client>(new mqtt::async_client(uri, id));
	} catch (const mqtt::exception& e) {
...
void MqttBridge::connect() {
	...
	std::string as_client = clientId.empty() ? "" : std::string(" as '") + clientId + std::string("'");
	RCLCPP_INFO(
	  get_logger(), "Connecting to broker at '%s'%s ...", client_->get_server_uri().c_str(), as_client.c_str());
...
void MqttBridge::newMqtt2RosBridge(atos_interfaces::srv::NewMqtt2RosBridge::Request::SharedPtr request,
								   atos_interfaces::srv::NewMqtt2RosBridge::Response::SharedPtr response) {
	...
	try {
		...
		RCLCPP_INFO(get_logger(), "Subscribed to MQTT topic '%s'", mqtt_topic_to_subscribe.c_str());
		response->success = true;
	}
	// Catch exception if the topic is already subscribed
	catch (const mqtt::exception& e) {
		RCLCPP_WARN(get_logger(), "Failed to subscribe MQTT topic '%s': %s", mqtt_topic_to_subscribe.c_str(), e.what());
		response->success = false;
	}
}

void MqttBridge::newRos2MqttBridge(atos_interfaces::srv::NewRos2MqttBridge::Request::SharedPtr request,
...
void MqttBridge::onCustomCommandActionMsg(
  const CustomCommandAction::message_type::SharedPtr custom_command_action_msg) {
	if (custom_command_action_msg->type == "v2x") {
		RCLCPP_INFO(this->get_logger(), "Received V2X message on %s topic", CustomCommandAction::topicName.c_str());
		this->onMessage<std::string>(custom_command_action_msg->content, "v2x", v2xToJson);
	}
}
...
json MqttBridge::v2xToJson(std::string msg_content) {
	...
	if (j["message_type"] == "DENM") {
		j["detection_time"] = std::chrono::system_clock::now().time_since_epoch().count();
	}
	return j;
}
