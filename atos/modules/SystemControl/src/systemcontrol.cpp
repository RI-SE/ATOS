...
#define SYSTEM_CONTROL_NO_OF_MODULES_IN_USE                                                                            \
	2 // TODO Create a file containing a list of which modules should be used. Check with this list to see if each
	  // module has responded.
...
const int64_t SystemControl::getQueueEmptyPollPeriod() {
	...
	if (SystemControlState == SERVER_STATE_INWORK) {
		...
		if (failedCommand == COMM_INIT && PreviousSystemControlCommand == InitializeScenario_0) {
			...
			RCLCPP_ERROR(get_logger(),
						 "Unhandled FAILURE (command: %u) reply in state %s",
						 msg->data,
						 SystemControlStatesArr[SystemControlState]);
		}
	} else {
		RCLCPP_WARN(get_logger(),
					"Received unexpected FAILURE (command: %u) reply in state %s",
					msg->data,
					SystemControlStatesArr[SystemControlState]);
		// TODO: React more?
	}
...
void SystemControl::onBackToStartResponse(const BackToStartResponse::message_type::SharedPtr msg) {
	if (msg->data == BTS_FAIL) {
		...
		SystemControlSendControlResponse(SYSTEM_CONTROL_RESPONSE_CODE_OK,
										 ...
										 1,
										 &ClientSocket,
										 0); // TODO: Send SMCP response the right way(?)
	} else if (msg->data == BTS_PASS) {
		RCLCPP_DEBUG(get_logger(), "Back-to-start result: %d", msg->data);
		memset(ControlResponseBuffer, 0, sizeof(ControlResponseBuffer));
		SystemControlSendControlResponse(SYSTEM_CONTROL_RESPONSE_CODE_OK, "BTS:", "1", 1, &ClientSocket, 0);
	} else {
	...
	if (ModeU8 == 0) {
		if (ClientSocket <= 0) {
			...
			if (USE_LOCAL_USER_CONTROL == 0) {
				...
				SystemControlCreateProcessChannel(
				  UserControlIPchar, SYSTEM_CONTROL_PROCESS_PORT, &RVSSChannelSocket, &RVSSChannelAddr);
			}
			if (USE_LOCAL_USER_CONTROL == 1) {
				ClientResult =
				  SystemControlConnectServer(&ClientSocket, LOCAL_USER_CONTROL_IP, LOCAL_USER_CONTROL_PORT);
				SystemControlCreateProcessChannel(
				  LOCAL_USER_CONTROL_IP, SYSTEM_CONTROL_PROCESS_PORT, &RVSSChannelSocket, &RVSSChannelAddr);
			}
		...
		if (ClientResult <= -1) {
			...
			if (CmdPtr != NULL) {
				...
				if (HTTPHeader.Host[0] == '\0') {
					...
					if (StartPtr == NULL || strchr(StartPtr, ')') == NULL)
						RCLCPP_WARN(get_logger(), "Received command not conforming to MSCP standards");
					else {
						StartPtr++;
						while (StopPtr != NULL) {
							...
							// Otherwise, check if the comma we found was inside quotation marks
								...
								if (OpeningQuotationMarkPtr == NULL) {
									...
									// A quotation mark was found and it was before the next comma: find the closing
									// quotation mark
									...
									if (ClosingQuotationMarkPtr == NULL) {
										CmdPtr	= NULL;
										StopPtr = NULL;
										RCLCPP_WARN(get_logger(), "Received MSCP command with single quotation mark");
										break;
									} else {
							...
							if (SystemControlArgument[CurrentInputArgCount][0] != '\0') {
								CurrentInputArgCount++; // In case the argument was empty, don't add it to the argument
														// count
							}
						}
					...
					RCLCPP_INFO(
					  get_logger(), "Request specified host <%s> not among known local addresses", HTTPHeader.Host);
				}
			} else {
				RCLCPP_WARN(get_logger(),
			...
			RCLCPP_WARN(get_logger(), "Ignored received TCP message which was too large to handle");
								  ...
								  CurrentTimeU64 = (uint64_t)CurrentTimeStruct.tv_sec*1000 +
	...
	if (SystemControlState == SERVER_STATE_INWORK) {
		if (SystemControlCommand == AbortScenario_0) {
			...
			RCLCPP_DEBUG(get_logger(),
						 "State: %s, OBCState: %s, PreviousCommand: %s",
						 SystemControlStatesArr[SystemControlState],
			...
			SystemControlSendControlResponse(
			  SYSTEM_CONTROL_RESPONSE_CODE_OK, "GetServerStatus:", ControlResponseBuffer, 2, &ClientSocket, 0);
			...
			bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
			SystemControlSendControlResponse(
			  SYSTEM_CONTROL_RESPONSE_CODE_INCORRECT_STATE, "", ControlResponseBuffer, 0, &ClientSocket, 0);
			SystemControlCommand = PreviousSystemControlCommand;
		}
	}
}
	...
	if (timercmp(&tvTime, &nextRVSSSendTime, >)) {
		...
		if (RVSSChannelSocket != 0 && RVSSConfigU32 > 0) {
			...
			if (RVSSConfigU32 & RVSS_TIME_CHANNEL) {
				...
				UtilSendUDPData((uint8_t*)module_name.c_str(),
								&RVSSChannelSocket,
								&RVSSChannelAddr,
								(uint8_t*)RVSSData,
								RVSSMessageLengthU32,
								0);
			}

			if (RVSSConfigU32 & RVSS_ATOS_CHANNEL) {
				SystemControlBuildRVSSATOSChannelMessage(RVSSData, &RVSSMessageLengthU32, SystemControlState, 0);
				UtilSendUDPData((uint8_t*)module_name.c_str(),
								&RVSSChannelSocket,
								&RVSSChannelAddr,
								(uint8_t*)RVSSData,
...
void SystemControl::processUserCommand() {
	switch (SystemControlCommand) {
			// can you access GetServerParameterList_0, GetServerParameter_1, SetServerParameter_2 and DISarmScenario
			// and Exit from the GUI
		case Idle_0:
			break;
		...
		case GetServerParameterList_0:
			...
			SystemControlSendControlResponse(strlen(ParameterListchar) > 0 ? SYSTEM_CONTROL_RESPONSE_CODE_OK
																		   : SYSTEM_CONTROL_RESPONSE_CODE_NO_DATA,
											 "GetServerParameterList:",
											 ParameterListchar,
											 strlen(ParameterListchar),
											 &ClientSocket,
											 0);
			break;
		...
		case GetServerParameter_1:
			if (CurrentInputArgCount == CommandArgCount) {
				SystemControlCommand = Idle_0;
				bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
				SystemControlGetServerParameter(
				  SystemControlArgument[0], ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE, 0);
				SystemControlSendControlResponse(strlen(ControlResponseBuffer) > 0
		...
		case SetServerParameter_2:
			if (CurrentInputArgCount == CommandArgCount) {
				...
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "SetServerParameter:", ControlResponseBuffer, 0, &ClientSocket, 0);
			} else {
		...
		case CreateDirectory_1:
			if (CurrentInputArgCount == CommandArgCount) {
				...
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "CreateDirectory:", ControlResponseBuffer, 1, &ClientSocket, 0);
		...
		case GetDirectoryContent_1:
			if (CurrentInputArgCount == CommandArgCount) {
				...
				SystemControlCheckFileDirectoryExist(SystemControlArgument[0], ControlResponseBuffer, 0);
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "GetDirectoryContent:", ControlResponseBuffer, 1, &ClientSocket, 0);
				if (ControlResponseBuffer[0] == FOLDER_EXIST) {
					...
					SystemControlFileDownloadResponse(
					  SYSTEM_CONTROL_RESPONSE_CODE_OK, "SubGetDirectoryContent:", FileLengthI32, &ClientSocket, 0);
					SystemControlSendFileContent(&ClientSocket,
												 "dir.info",
		...
		case DeleteTrajectory_1:
			if (CurrentInputArgCount == CommandArgCount) {
				...
				memset(ControlResponseBuffer, 0, sizeof(ControlResponseBuffer));
				*ControlResponseBuffer =
				  SystemControlDeleteTrajectory(SystemControlArgument[0], sizeof(SystemControlArgument[0]));
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "DeleteTrajectory:", ControlResponseBuffer, 1, &ClientSocket, 0);
			} else {
				RCLCPP_ERROR(get_logger(),
		...
		case DeleteGeofence_1:
			if (CurrentInputArgCount == CommandArgCount) {
				...
				*ControlResponseBuffer =
				...
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "DeleteGeofence:", ControlResponseBuffer, 1, &ClientSocket, 0);
			} else {
				RCLCPP_ERROR(get_logger(),
		...
		case DeleteFileDirectory_1:
			if (CurrentInputArgCount == CommandArgCount) {
				...
				*ControlResponseBuffer =
				...
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "DeleteFileDirectory:", ControlResponseBuffer, 1, &ClientSocket, 0);
			} else {
				RCLCPP_ERROR(get_logger(),
		...
		case ClearTrajectories_0:
			if (CurrentInputArgCount == CommandArgCount) {
				...
				*ControlResponseBuffer = SystemControlClearTrajectories();
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "ClearTrajectories:", ControlResponseBuffer, 1, &ClientSocket, 0);
			} else {
				RCLCPP_ERROR(get_logger(),
		...
		case ClearGeofences_0:
			if (CurrentInputArgCount == CommandArgCount) {
				...
				*ControlResponseBuffer = SystemControlClearGeofences();
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "ClearGeofences:", ControlResponseBuffer, 1, &ClientSocket, 0);
			} else {
				RCLCPP_ERROR(get_logger(),
		...
		case ClearObjects_0:
			if (CurrentInputArgCount == CommandArgCount) {
				...
				*ControlResponseBuffer = SystemControlClearObjects();
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "ClearObjects:", ControlResponseBuffer, 1, &ClientSocket, 0);
			} else {
				RCLCPP_ERROR(get_logger(),
		...
		case DownloadFile_1:
			if (CurrentInputArgCount == CommandArgCount) {
				...
				SystemControlCheckFileDirectoryExist(SystemControlArgument[0], ControlResponseBuffer, 0);
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "DownloadFile:", ControlResponseBuffer, 1, &ClientSocket, 0);
				if (ControlResponseBuffer[0] == FILE_EXIST) {
					bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
					FileLengthI32 = SystemControlBuildFileContentInfo(SystemControlArgument[0], 0);
					SystemControlFileDownloadResponse(
					  SYSTEM_CONTROL_RESPONSE_CODE_OK, "SubDownloadFile:", FileLengthI32, &ClientSocket, 0);
					SystemControlSendFileContent(&ClientSocket,
				...
				if (ControlResponseBuffer[0] == FOLDER_EXIST) {
					...
					for (int i = 0; i < rows; i++) {
						...
						if (*RowBuffer == 'F') {
							memset(InPath, 0, MAX_PATH_LENGTH);
							if (SystemControlCommand == DownloadTrajFiles_0)
								strcat(InPath, ATOS_TRAJ_DIRECTORY_STRING);
							else if (SystemControlCommand == DownloadDirectoryContent_1)
							...
							SystemControlFileDownloadResponse(
							  SYSTEM_CONTROL_RESPONSE_CODE_OK, functionReturnName, FileLengthI32, &ClientSocket, 0);
							SystemControlSendFileContent(&ClientSocket,
														 InPath,
														 STR_SYSTEM_CONTROL_TX_PACKET_SIZE,
														 SystemControlDirectoryInfo.info_buffer,
					...
					SystemControlSendControlResponse(
					  SYSTEM_CONTROL_RESPONSE_CODE_OK, functionReturnName, ControlResponseBuffer, 3, &ClientSocket, 0);
				}
				SystemControlCommand = Idle_0;
			} else {
				RCLCPP_ERROR(get_logger(), "Wrong parameter count in GetDirectoryContent(path)!");
		...
		case UploadFile_4:
			if (CurrentInputArgCount == CommandArgCount) {
				...
				SystemControlUploadFile(SystemControlArgument[0],
										SystemControlArgument[1],
										SystemControlArgument[2],
										SystemControlArgument[3],
										ControlResponseBuffer,
										RxFilePath,
										0);
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "UploadFile:", ControlResponseBuffer, 1, &ClientSocket, 0);
				...
				if (ControlResponseBuffer[0] == SERVER_PREPARED_BIG_PACKET_SIZE) // Server is ready to receive data
				{
					...
					SystemControlReceiveRxData(&ClientSocket,
											   RxFilePath,
											   SystemControlArgument[1],
											   STR_SYSTEM_CONTROL_RX_PACKET_SIZE,
											   ControlResponseBuffer,
											   0);
				...
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "SubUploadFile:", ControlResponseBuffer, 1, &ClientSocket, 0);
		...
		case InitializeScenario_0:
			if (SystemControlState == SERVER_STATE_IDLE && objectControlState == OBC_STATE_IDLE) {
				...
				bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "InitializeScenario:", ControlResponseBuffer, 0, &ClientSocket, 0);
		...
		case ConnectObject_0:
			if (SystemControlState == SERVER_STATE_IDLE && objectControlState == OBC_STATE_INITIALIZED) {
				...
				bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "ConnectObject:", ControlResponseBuffer, 0, &ClientSocket, 0);
		...
		case DisconnectObject_0:
			if (SystemControlState == SERVER_STATE_IDLE) {
				...
				bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "DisconnectObject:", ControlResponseBuffer, 0, &ClientSocket, 0);
		...
		case ArmScenario_0:
			if (SystemControlState == SERVER_STATE_IDLE && objectControlState == OBC_STATE_CONNECTED) {
				...
				bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "ArmScenario:", ControlResponseBuffer, 0, &ClientSocket, 0);
		...
		case RemoteControl_1:
			...
			SystemControlCommand = Idle_0;
			SystemControlSendControlResponse(
			  responseCode, "RemoteControl:", ControlResponseBuffer, 0, &ClientSocket, 0);
			break;
		case RemoteControlManoeuvre_2:
			...
			SystemControlCommand = Idle_0;
			SystemControlSendControlResponse(
			  responseCode, "RemoteControlManoeuvre:", ControlResponseBuffer, 0, &ClientSocket, 0);
			break;
		case GetObjectEnableStatus_1:
			if (CurrentInputArgCount == CommandArgCount) {
				if (SystemControlState == SERVER_STATE_IDLE) {
					...
					if (inet_pton(AF_INET, SystemControlArgument[0], &enableCommand.objectIP) != -1) {
						...
						SystemControlSendControlResponse(
						  responseCode, "GetObjectEnableStatus:", ControlResponseBuffer, 1, &ClientSocket, 0);
					} else
						responseCode = SYSTEM_CONTROL_RESPONSE_CODE_ERROR;
				} else
					responseCode = SYSTEM_CONTROL_RESPONSE_CODE_INCORRECT_STATE;
			...
			if (responseCode != SYSTEM_CONTROL_RESPONSE_CODE_OK)
				SystemControlSendControlResponse(
				  responseCode, "GetObjectEnableStatus:", ControlResponseBuffer, 0, &ClientSocket, 0);
			SystemControlCommand = Idle_0;
			break;
		case StartScenario_1:
			if (CurrentInputArgCount == CommandArgCount) {
				if (SystemControlState == SERVER_STATE_IDLE && objectControlState == OBC_STATE_ARMED) // Temporary!
				{
					...
					RCLCPP_INFO(get_logger(), "Sending START <%s> (delayed +%u ms)", pcBuffer, DelayedStartU32);
					try {
					...
					bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
					SystemControlSendControlResponse(
					  SYSTEM_CONTROL_RESPONSE_CODE_OK, "StartScenario:", ControlResponseBuffer, 0, &ClientSocket, 0);
					SystemControlState = SERVER_STATE_INWORK;
		...
		case stop_0:
			try {
				...
				bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "stop:", ControlResponseBuffer, 0, &ClientSocket, 0);
			} catch (...) {
				this->disconnectPub.publish(Disconnect::message_type());
		...
		case AbortScenario_0:
			try {
				...
				if (ClientSocket >= 0) {
					...
					SystemControlSendControlResponse(
					  SYSTEM_CONTROL_RESPONSE_CODE_OK, "AbortScenario:", ControlResponseBuffer, 1, &ClientSocket, 0);
				}
			} catch (...) {
		...
		case ClearAllScenario_0:
			try {
				...
				if (ClientSocket >= 0) {
					...
					ControlResponseBuffer[0] = 1;
					SystemControlSendControlResponse(
					  SYSTEM_CONTROL_RESPONSE_CODE_OK, "ClearAllScenario:", ControlResponseBuffer, 1, &ClientSocket, 0);
				}
			} catch (...) {
		...
		case Exit_0:
			try {
				...
				bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
				SystemControlSendControlResponse(
				  SYSTEM_CONTROL_RESPONSE_CODE_OK, "Exit:", ControlResponseBuffer, 0, &ClientSocket, 0);
				close(ClientSocket);
				ClientSocket = -1;
				if (USE_LOCAL_USER_CONTROL == 0) {
					close(ServerHandle);
...
SystemControl::SystemControlCommand_t SystemControl::SystemControlFindCommand(
  ...
  int* CommandArgCount) {
	...
	for (command = Idle_0; command != nocommand; command = (SystemControlCommand_t)(command + 1)) {
		...
		strncpy(SystemControlStrippedCommand,
				SystemControlCommandsArr[(int)command],
				(uint64_t)strchr(SystemControlCommandsArr[(int)command], '_') -
				  (uint64_t)SystemControlCommandsArr[(int)command]);
		strncpy(SystemControlCommandArgCnt,
				strchr(SystemControlCommandsArr[(int)command], '_') + 1,
...
/*!
 * \brief SystemControlReceiveUserControlData Performs similarly to the recv function (see manpage for recv) except that
 * it only fills the input data buffer with messages ending with ";\r\n\r\n" and saves any remaining data in a local
 ...
 * \return Number of bytes printed to dataBuffer where 0 means that the connection has been severed. A return value of
 * -1 constitutes an error with the appropriate errno has been set (see manpage for recv) with the addition of
...
void SystemControl::SystemControlSendControlResponse(U16 ResponseStatus,
													 const char* ResponseString,
													 const char* ResponseData,
													 I32 ResponseDataLength,
													 I32* Sockfd,
													 U8 Debug) {
	int i, n, j, t;
	char Length[4];
...
void SystemControl::SystemControlFileDownloadResponse(U16 ResponseStatus,
													  const char* ResponseString,
													  I32 ResponseDataLength,
													  I32* Sockfd,
													  U8 Debug) {
	...
	UtilSendTCPData((uint8_t*)"System Control",
					(uint8_t*)Data,
					MSCP_RESPONSE_DATALENGTH_BYTES + MSCP_RESPONSE_STATUS_CODE_BYTES + strlen(ResponseString),
					Sockfd,
					0);
}

I32 SystemControl::SystemControlBuildControlResponse(U16 ResponseStatus,
													 char* ResponseString,
													 char* ResponseData,
													 I32 ResponseDataLength,
													 U8 Debug) {
...
I32 SystemControl::SystemControlInitServer(int* ClientSocket, int* ServerHandle, struct in_addr* ip_addr) {
	...
	*ServerHandle = socket(AF_INET, SOCK_STREAM, 0);
	...
	RCLCPP_INFO(
	  get_logger(), "Connection established: %s:%i", inet_ntoa(cli_addr.sin_addr), htons(command_server_addr.sin_port));
...
void SystemControl::SystemControlCreateProcessChannel(const char* name,
													  const U32 port,
													  I32* sockfd,
...
I32 SystemControl::SystemControlGetServerParameter(char* ParameterName, char* ReturnValue, U32 BufferLength, U8 Debug) {
	...
	switch (parameter) {
		...
		case CONFIGURATION_PARAMETER_ORIGIN_LATITUDE:
			if ((result = DataDictionaryGetNumberOfObjects(&numberOfObjects)) != READ_OK) {
				RCLCPP_ERROR(get_logger(),
							 "Data dictionary number of objects read error"
			...
			if ((result = DataDictionaryGetObjectTransmitterIDs(object_transmitter_ids, numberOfObjects)) != READ_OK) {
				RCLCPP_ERROR(get_logger(),
							 "Data dictionary TransmitterID read error"
							 "- cannot set origin on objects");
				break;
			}
			...
			for (unsigned int i = 0; i < numberOfObjects; ++i) {
				if (object_transmitter_ids[i] == 0) {
					...
					if (DataDictionaryGetOrigin(object_transmitter_ids[i], &origin) != READ_OK) {
						RCLCPP_ERROR(get_logger(),
									 "Data dictionary origin read error"
									 "- cannot read the origin for object %u",
					...
					if (DataDictionarySetOrigin(&object_transmitter_ids[i], &origin) != WRITE_OK) {
						RCLCPP_ERROR(get_logger(),
									 "Data dictionary origin write error"
									 "- cannot set the origin for object %u",
									 object_transmitter_ids[i]);
						result = UNDEFINED;
						break;
					}
...
I32 SystemControl::SystemControlUploadFile(const char* Filename,
										   const char* FileSize,
...
I32 SystemControl::SystemControlReceiveRxData(I32* sockfd,
											  const char* Path,
											  const char* FileSize,
											  const char* PacketSize,
											  char* ReturnValue,
	...
	U32 TotalRxCount = 0, TransmissionCount = (U32)(FileSizeU32 / PacketSizeU16),
		RestCount = FileSizeU32 % PacketSizeU16;
	...
	if (fd != NULL) {
		...
		while (TotalRxCount < FileSizeU32 && TimeDiff < 3000) {
			...
			if (ClientStatus > 0) {
				...
				if (Debug) {
					printf(
					  "%d, %d, %d, %d, %d, %d :", i, ClientStatus, TotalRxCount, TimeDiff, PacketSizeU16, FileSizeU32);
					for (j = 0; j < 10; j++)
						printf("%x-", RxBuffer[j]);
					printf("...\n");
				}
...
I32 SystemControl::SystemControlSendFileContent(I32* sockfd,
												const char* Path,
												const char* PacketSize,
												char* ReturnValue,
												U8 Remove,
	...
	if (fd != NULL) {
		...
		RCLCPP_INFO(get_logger(),
					"Sent file: %s, total size = %d, transmissions = %d",
					CompletePath,
					(PacketSizeU16 * TransmissionCount + RestCount),
					i + 1);
	}
...
*/
I32 SystemControl::SystemControlBuildRVSSTimeChannelMessage(char* RVSSData, U32* RVSSDataLengthU32, U8 Debug) {
...
/*
SystemControlBuildRVSSATOSChannelMessage builds a message from OBCState in DataDictionary and SysCtrlState. The message
is stored in *RVSSData. See the architecture document for the protocol of RVSS.
...
- Debug enable(1)/disable(0) debug printouts (Not used)
*/
I32 SystemControl::SystemControlBuildRVSSATOSChannelMessage(char* RVSSData,
															U32* RVSSDataLengthU32,
															U8 SysCtrlState,
															U8 Debug) {
...
int32_t SystemControl::SystemControlSendRVSSMonitorChannelMessages(int* socket, struct sockaddr_in* addr) {
	...
	if (DataDictionaryGetNumberOfObjects(&numberOfObjects) != READ_OK) {
		RCLCPP_ERROR(get_logger(), "Data dictionary number of objects read error - RVSS messages cannot be sent");
		return -1;
	}
	...
	if (DataDictionaryGetObjectTransmitterIDs(transmitterIDs, numberOfObjects) != READ_OK) {
		free(transmitterIDs);
		RCLCPP_ERROR(get_logger(), "Data dictionary transmitter ID read error - RVSS messages cannot be sent");
		return -1;
	}
		...
		if (transmitterIDs[i] != 0) {
			...
			if (DataDictionaryGetObjectIPByTransmitterID(transmitterIDs[i], &monitorData.ClientIP) != READ_OK) {
				...
				RCLCPP_ERROR(
				  ...
				  transmitterIDs[i]);
				retval = -1;
			} else if (UtilObjectDataToString(monitorData,
											  monitorDataString,
