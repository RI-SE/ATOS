...
void TrajectoryPublisher::publishChunk() {
	...
	if (chunk != lastPublishedChunk || (chunkLength == 0ms && now - lastPublishTime > 1s)) {
...
TrajectoryPublisher::Chunk TrajectoryPublisher::extractChunk(std::chrono::steady_clock::duration beginTime,
															 std::chrono::steady_clock::duration endTime) {
	using std::chrono::steady_clock;
	auto ret = Chunk(traj->points.cbegin(), traj->points.cend());

	ret.first = std::upper_bound(
	  traj->points.cbegin(),
	  traj->points.cend(),
	  beginTime,
	  [](const steady_clock::duration& dur, const Trajectory::TrajectoryPoint& pt) { return pt.getTime() > dur; });
	if (ret.first == traj->points.cend()) {
		return ret;
	}
	ret.second = std::upper_bound(
	  ret.first,
	  traj->points.cend(),
	  endTime,
	  [](const steady_clock::duration& dur, const Trajectory::TrajectoryPoint& pt) { return pt.getTime() > dur; });
...
nav_msgs::msg::Path TrajectoryPublisher::chunkToPath(Chunk chunk, std::chrono::steady_clock::time_point relTimeOffset) {
	...
	std::transform(
	  chunk.first, chunk.second, std::back_inserter(ret.poses), [&](const Trajectory::TrajectoryPoint& pt) {
