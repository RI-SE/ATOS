...
#define DISABLE_CONTROL 1
#define SHOW_VIEWER 0
#define THREADS 0
#define RECORD 0

std::shared_ptr<EsminiAdapter> EsminiAdapter::me = nullptr;
std::unordered_map<int, std::string> EsminiAdapter::atosIDToObjectName =
    std::unordered_map<int, std::string>();
std::unordered_map<std::string, int> EsminiAdapter::objectNameToAtosId =
    std::unordered_map<std::string, int>();
std::map<uint32_t, ATOS::Trajectory> EsminiAdapter::atosObjectIdToTraj =
    std::map<uint32_t, ATOS::Trajectory>();
std::unordered_map<int, int> EsminiAdapter::atosIdToEsminiId =
    std::unordered_map<int, int>();

std::unordered_map<uint32_t, std::shared_ptr<ROSChannels::Monitor::Sub>>
    EsminiAdapter::monrSubscribers =
        std::unordered_map<uint32_t, std::shared_ptr<Monitor::Sub>>();
std::shared_ptr<rclcpp::Service<ObjectTrajectorySrv>>
    EsminiAdapter::objectTrajectoryService =
        std::shared_ptr<rclcpp::Service<ObjectTrajectorySrv>>();
std::shared_ptr<rclcpp::Service<TestOriginSrv>>
    EsminiAdapter::testOriginService =
        std::shared_ptr<rclcpp::Service<TestOriginSrv>>();
geographic_msgs::msg::GeoPose EsminiAdapter::testOrigin =
    geographic_msgs::msg::GeoPose();

EsminiAdapter::EsminiAdapter()
    : Module(moduleName), storyBoardElementStateChangePub(*this),
      ...
      applyTrajTransform(false), testOriginSet(false) {
  oscFilePathClient_cb_group_ =
      this->create_callback_group(rclcpp::CallbackGroupType::Reentrant);
  objectIdsClient_cb_group_ =
      this->create_callback_group(rclcpp::CallbackGroupType::Reentrant);
  oscFilePathClient_ =
      create_client<atos_interfaces::srv::GetOpenScenarioFilePath>(
          ServiceNames::getOpenScenarioFilePath,
          rmw_qos_profile_services_default, oscFilePathClient_cb_group_);
  objectIdsClient_ = create_client<atos_interfaces::srv::GetObjectIds>(
      ServiceNames::getObjectIds, rmw_qos_profile_services_default,
      objectIdsClient_cb_group_);
...
std::filesystem::path EsminiAdapter::getOpenDriveFile() {
  ...
  if (SE_GetODRFilename() != nullptr) {
    ...
    RCLCPP_INFO(me->get_logger(), "Got ODR file %s from scenario",
                odrFilePath.string().c_str());
  } else {
  ...
  if (odrFilePath.is_absolute()) {
    ...
    if (!std::filesystem::exists(joinedPath)) {
      throw std::runtime_error("ODR file " + joinedPath.string() +
                               " does not exist");
    } else {
      RCLCPP_INFO(me->get_logger(), "Found ODR file at %s",
                  joinedPath.string().c_str());
    }
    return joinedPath;
  }
}
...
/*!
 * \brief Creates an instance and initialize esmini if none exists, otherwise
 * returns the existing instance. \return the sole EsminiAdapter instance.
 */
std::shared_ptr<EsminiAdapter> EsminiAdapter::instance() {
  if (me == nullptr) {
    ...
    me->connectedObjectIdsSub = ROSChannels::ConnectedObjectIds::Sub(
        *me, &EsminiAdapter::onConnectedObjectIdsMessage);
    me->exitSub =
        ROSChannels::Exit::Sub(*me, &EsminiAdapter::onStaticExitMessage);
    me->stateChangeSub = ROSChannels::StateChange::Sub(
        *me, &EsminiAdapter::onStaticStateChangeMessage);
  }
  return me;
}

/*!
 * \brief This callback is executed when objects are connected, and creates
 * Monitor subscribers for all connected objects. \param msg Array of object IDs
 */
void EsminiAdapter::onConnectedObjectIdsMessage(
    const ConnectedObjectIds::message_type::SharedPtr msg) {
  for (uint32_t id : msg->ids) {
    if (me->monrSubscribers.find(id) == me->monrSubscribers.end()) {
      me->monrSubscribers[id] = std::make_shared<Monitor::Sub>(
          *me, id, std::bind(&EsminiAdapter::onMonitorMessage, me, _1, id));
    }
  }
}

/**
 * @brief To ensure that EsminiAdapter follows the states, we execute actions
 * only when going from IDLE to INITIALIZED, from ARMED to RUNNING and from any
 * state to ABORTING. We do this instead of subscribing to "/init", "/start",
 * etc., because we only want to execute the actions once when changing to these
 * states.
 *
 * @param msg StateChange message
 */
void EsminiAdapter::onStaticStateChangeMessage(
    const ROSChannels::StateChange::message_type::SharedPtr msg) {
  int prevState = msg->prev_state;
  int currentState = msg->current_state;

  if (prevState == OBCState_t::OBC_STATE_ARMED &&
      currentState == OBCState_t::OBC_STATE_RUNNING) {
...
void EsminiAdapter::fetchOSCFilePath() {
  ...
  auto callback =
      [&](rclcpp::Client<
          atos_interfaces::srv::GetOpenScenarioFilePath>::SharedFuture future) {
  ...
  auto request = std::make_shared<
      atos_interfaces::srv::GetOpenScenarioFilePath::Request>();
  auto future =
      me->oscFilePathClient_->async_send_request(request, std::move(callback));
...
void EsminiAdapter::handleStartCommand() {
  if (SE_Init(me->oscFilePath.c_str(), ENABLE_CONTROL, SHOW_VIEWER, THREADS,
              RECORD) < 0) {
    throw std::runtime_error("Failed to initialize esmini with scenario file " +
                             me->oscFilePath.string());
  }
  // Handle triggers and story board element changes
  SE_RegisterStoryBoardElementStateChangeCallback(
      &handleStoryBoardElementChange);
 ...
 * 		If story board element is an action, and the action is supported,
 * the action is run. \param name Name of the StoryBoardElement whose state has
 * changed. \param type Possible values: STORY_BOARD = 1, STORY = 2, ACT = 3,
 * MANEUVER_GROUP = 4, MANEUVER = 5, EVENT = 6, ACTION = 7,
 * UNDEFINED_ELEMENT_TYPE = 0. \param state new state, possible values: STANDBY
 * = 1, RUNNING = 2, COMPLETE = 3, UNDEFINED_ELEMENT_STATE = 0.
 */
void EsminiAdapter::handleStoryBoardElementChange(const char *name, int type,
                                                  int state,
                                                  const char *full_path) {
  RCLCPP_INFO(
      me->get_logger(),
      "Storyboard state changed! Name: %s, Type: %d, State: %d, Full path: %s",
      name, type, state, full_path);
...
/*!
 * \brief Utility function to convert a ROS MONR message to Esmini
 *representation and report the object position to Esmini \param monr ROS
 *Monitor message of an object \param id The object ID to which the monr belongs
 */
void EsminiAdapter::reportObjectPosition(
    const Monitor::message_type::SharedPtr monr, uint32_t esminiObjectId) {
  ...
  SE_ReportObjectPos(esminiObjectId, timestamp, pos.x, pos.y, pos.z, yaw, pitch,
                     roll);
  SE_ReportObjectSpeed(esminiObjectId, speed.x);
}

/*!
 * \brief Callback for MONR messages, reports the object position to esmini and
 * advances the simulation time \param monr ROS Monitor message of an object
 * \param id The object ID to which the monr belongs
 */
void EsminiAdapter::onMonitorMessage(
    const Monitor::message_type::SharedPtr monr, uint32_t ATOSObjectId) {
  if (auto idMapping = atosIdToEsminiId.find(ATOSObjectId);
      idMapping != atosIdToEsminiId.end()) {
    reportObjectPosition(monr,
                         idMapping->second); // Report object position to esmini
    SE_Step(); // Advance the "simulation world"-time
  } else {
    RCLCPP_WARN(me->get_logger(),
                "Received MONR message for object with ATOS Object ID %d, but "
                "no such object exists in the scenario",
                ATOSObjectId);
  }
}
 ...
 *	creates a trajectory consisting of trajectory points, one for each
 *timestep. Note: If there is no difference between consecutive states, the
 *trajectory point is not added. \param id The object ID to which the trajectory
 *belongs \param states A vector of object states \return A trajectory
 *consisting of trajectory points, one for each state.
 */
ATOS::Trajectory EsminiAdapter::getTrajectoryFromObjectState(
    uint32_t id, std::vector<SE_ScenarioObjectState> &states) {
  ...
  auto saveTp = [&](auto &state, auto &prevState) {
    ...
    tp.setCurvature(0); // TODO: implement support for different curvature, now
    ...
    if (state.timestamp != prevState.timestamp) {
      tp.setLongitudinalAcceleration((currLonVel - prevLonVel) /
                                     (state.timestamp - prevState.timestamp));
      tp.setLateralAcceleration((currLatVel - prevLatVel) /
                                (state.timestamp - prevState.timestamp));
    } else {
  ...
  if (states.size() > 1) {
    for (auto it = states.begin() + 1; it != states.end(); ++it) {
      if (it->x == (it - 1)->x &&
          it->y == (it - 1)->y && // Nothing interesting happens within 1
                                  // timestep, skip
  ...
  if (trajectory.points.size() == 0) {
    saveTp(states.front(),
           states.front()); // Only one state or no points, save it.
  }
...
std::string EsminiAdapter::projStrFromGeoReference(RM_GeoReference &geoRef) {
  ...
  RCLCPP_DEBUG(me->get_logger(), "Created proj string: %s",
               projStringFrom.c_str());
  return projStringFrom;
}

/*!
 * \brief Returns object states for each timestep by simulating the loaded
 *scenario. The simulation is stopped if there is no vehicle movement and at
 *least MIN_SCENARIO_TIME has passed or if more than MAX_SCENARIO_TIME has
 *passed. Inspired by ScenarioGateway::WriteStatesToFile from esmini lib.
 *
 * \param timeStep Time step to use for generating the trajectories
 * \param endTime End time of the simulation
 * \param states The return map of ids mapping to the respective object states
 *at different timesteps \return A map of object states, where the key is the
 *object ID and the value is a vector of states
 */
void EsminiAdapter::getObjectStates(
  ...
  auto pushCurrentState = [&](auto &st, auto &index) {
    ...
    // SE_SetAlignModeZ(id, 0); // Disable Z-alignment, not implemented in
    // esmini yet
  ...
  // Populate States map with vector for each object containing the initial
  // state
  SE_StepDT(timeStep);
  accumTime += timeStep;
  ...
  while (!stopSimulation) {
    ...
    bool noMovement =
        std::all_of(states.begin(), states.end(),
                    [&](auto &pair) { return pair.second.back().speed < 0.1; });
    bool atLeastMinTimePassed = accumTime > MIN_SCENARIO_TIME;
    bool moreThanMaxTimePassed = accumTime > MAX_SCENARIO_TIME;
    stopSimulation =
        (noMovement && atLeastMinTimePassed) || moreThanMaxTimePassed;
  }
  if (accumTime > MAX_SCENARIO_TIME) {
    RCLCPP_WARN(me->get_logger(),
                "Scenario time limit reached, stopping simulation");
  } else if (accumTime < MIN_SCENARIO_TIME + timeStep) {
    RCLCPP_WARN(me->get_logger(),
                "Ran scenario for the minimum time %.2f, possibly no movement "
                "in scenario",
                MIN_SCENARIO_TIME);
  }
...
/*!
 * \brief Runs the esmini simulator with the xosc file and returns the
 * trajectories for each object \param timeStep Time step to use for generating
 * the trajectories \return A map of ids mapping to the respective trajectories
 */
std::map<uint32_t, ATOS::Trajectory>
EsminiAdapter::extractTrajectories(double timeStep) {
  ...
  for (auto &os : esminiIdToStates) {
    ...
    if (me->applyTrajTransform) {
      RCLCPP_DEBUG(me->get_logger(),
                   "Applying CRS transformation to trajectory for object %d",
                   id);
      me->crsTransformation->apply(traj.points);
    }
    esminiObjectIdToTraj.insert(
        std::pair<uint32_t, ATOS::Trajectory>(id, traj));
  }
...
void EsminiAdapter::runEsminiSimulation() {
  ...
  auto logFilePath = std::string(getenv("HOME")) +
                     std::string("/.astazero/ATOS/logs/esmini.log");
  ...
  RCLCPP_INFO(me->get_logger(), "Initializing esmini with scenario file %s",
              me->oscFilePath.c_str());
  if (SE_Init(me->oscFilePath.c_str(), DISABLE_CONTROL, SHOW_VIEWER, THREADS,
              RECORD) <
      0) { // Disable controllers, let DefaultController be used
    throw std::runtime_error("Failed to initialize esmini with scenario file " +
                             me->oscFilePath.string() +
                             ". For more information, see " + logFilePath +
                             ".");
  }
  auto odrFile = getOpenDriveFile();
  if (RM_Init(odrFile.c_str()) < 0) {
    throw std::runtime_error(
        std::string("Failed to initialize with odr file ").append(odrFile));
  }
  ...
  if (RM_GetOpenDriveGeoReference(&geoRef) == 0) {
    try {
      ...
      RCLCPP_INFO(me->get_logger(), "llh origin: %lf, %lf, %lf", llh_0[0],
                  llh_0[1], llh_0[2]);
      ...
      std::string projStringTo =
          "+proj=tmerc +lat_0=" + std::to_string(llh_0[0]) +
          " +lon_0=" + std::to_string(llh_0[1]) + " +datum=" + toDatum +
          " +units=m +no_defs";

      me->crsTransformation =
          std::make_shared<CRSTransformation>(projStringFrom, projStringTo);
    ...
    RCLCPP_WARN(me->get_logger(),
                "Failed to get OpenDRIVE geo reference from RoadManager");
  }
  RM_Close();

  auto response =
      std::make_shared<atos_interfaces::srv::GetObjectIds::Response>();
  auto request =
      std::make_shared<atos_interfaces::srv::GetObjectIds::Request>();

  std::promise<void> idsFetched;
  auto objectNameAndAtosIDsCallback =
      [&](rclcpp::Client<
          atos_interfaces::srv::GetObjectIds>::SharedFutureWithRequest future) {
        auto response = future.get();
        for (int i = 0; i < response.second->ids.size(); i++) {
          me->atosIDToObjectName[response.second->ids[i]] =
              response.second->names[i];
          me->objectNameToAtosId[response.second->names[i]] =
              response.second->ids[i];
        }
        idsFetched.set_value();
      };

  auto future = me->objectIdsClient_->async_send_request(
      request, std::move(objectNameAndAtosIDsCallback));
  ...
  std::map<uint32_t, ATOS::Trajectory> esminiIdToTraj =
      me->extractTrajectories(timeStep);
  ...
  for (auto &it : esminiIdToTraj) {
    ...
    if (auto idMapping = me->objectNameToAtosId.find(objectName);
        idMapping != me->objectNameToAtosId.end()) {
      auto atos_id = idMapping->second;
      me->atosObjectIdToTraj.emplace(atos_id, traj);
      me->atosIdToEsminiId.emplace(atos_id, esminiId);
      RCLCPP_INFO(me->get_logger(),
                  "Extracted trajectory for object %s with size %d", objectName,
                  traj.points.size());

      me->pathPublishers.emplace(atos_id, ROSChannels::Path::Pub(*me, atos_id));
      me->pathPublishers.at(atos_id).publish(traj.toPath());
      std::array<double, 3> llh_0 = {me->testOrigin.position.latitude,
                                     me->testOrigin.position.longitude,
                                     me->testOrigin.position.altitude};
      me->gnssPathPublishers.emplace(atos_id,
                                     ROSChannels::GNSSPath::Pub(*me, atos_id));
      me->gnssPathPublishers.at(atos_id).publish(traj.toGeoJSON(llh_0));
    } else {
      RCLCPP_DEBUG(me->get_logger(),
                   "Object %s is not an active object in the scenario",
                   objectName);
    }

    for (auto &tp : traj.points) {
      RCLCPP_DEBUG(me->get_logger(),
                   "Trajectory point: %lf, %lf, %lf, %lf, %ld", tp.getXCoord(),
                   tp.getYCoord(), tp.getZCoord(), tp.getHeading(),
                   tp.getTime().count());
    }
  }
...
void EsminiAdapter::onRequestObjectTrajectory(
  ...
  me->fetchOSCFilePath();
  if (me->runSimulation ||
      me->atosObjectIdToTraj.find(req->id) == me->atosObjectIdToTraj.end()) {
  ...
  try {
    res->trajectory =
        me->atosObjectIdToTraj.at(req->id).toCartesianTrajectory();
    res->success = true;
  } catch (std::out_of_range &e) {
    RCLCPP_ERROR(
        me->get_logger(),
        "Esmini trajectory service called, no trajectory found for object %d",
        req->id);
    res->success = false;
  }
}
 ...
 *			connecting to the message queue bus, setting up signal
 *handers etc. \param logLevel Level of the module log to be used. \return 0 on
 *success, -1 otherwise
 */
int EsminiAdapter::initializeModule() {
  int retval = 0;

  me->objectTrajectoryService = me->create_service<ObjectTrajectorySrv>(
      ServiceNames::getObjectTrajectory,
      std::bind(&EsminiAdapter::onRequestObjectTrajectory, _1, _2));
  me->testOriginService =
      me->create_service<atos_interfaces::srv::GetTestOrigin>(
          ServiceNames::getTestOrigin,
