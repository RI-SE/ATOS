...
std::unordered_map<uint32_t, std::shared_ptr<ROSChannels::Monitor::Sub>> EsminiAdapter::monrSubscribers =
  std::unordered_map<uint32_t, std::shared_ptr<Monitor::Sub>>();
std::shared_ptr<rclcpp::Service<ObjectTrajectorySrv>> EsminiAdapter::objectTrajectoryService =
  std::shared_ptr<rclcpp::Service<ObjectTrajectorySrv>>();
std::shared_ptr<rclcpp::Service<TestOriginSrv>> EsminiAdapter::testOriginService =
...
std::shared_ptr<EsminiAdapter> EsminiAdapter::instance() {
	if (me == nullptr) {
		...
		me->connectedObjectIdsSub =
		  ROSChannels::ConnectedObjectIds::Sub(*me, &EsminiAdapter::onConnectedObjectIdsMessage);
...
/*!
 * \brief This callback is executed when objects are connected, and creates Monitor subscribers for all connected
 * objects.
...
void EsminiAdapter::onConnectedObjectIdsMessage(const ConnectedObjectIds::message_type::SharedPtr msg) {
	for (uint32_t id : msg->ids) {
		if (me->monrSubscribers.find(id) == me->monrSubscribers.end()) {
			me->monrSubscribers[id] =
			  std::make_shared<Monitor::Sub>(*me, id, std::bind(&EsminiAdapter::onMonitorMessage, me, _1, id));
		}
	}
}
 ...
 * \param type Possible values: STORY_BOARD = 1, STORY = 2, ACT = 3, MANEUVER_GROUP = 4, MANEUVER = 5, EVENT = 6, ACTION
 * = 7, UNDEFINED_ELEMENT_TYPE = 0.
 * \param state new state, possible values: STANDBY = 1, RUNNING = 2, COMPLETE = 3, UNDEFINED_ELEMENT_STATE = 0.
 */
void EsminiAdapter::handleStoryBoardElementChange(const char* name, int type, int state, const char* full_path) {
...
void EsminiAdapter::reportObjectPosition(const Monitor::message_type::SharedPtr monr, uint32_t esminiObjectId) {
	...
	// Reporting to Esmini
#ifdef ESMINI_REPORT_OBJECT_POS_HAS_TIMESTAMP
...
void EsminiAdapter::onMonitorMessage(const Monitor::message_type::SharedPtr monr, uint32_t ATOSObjectId) {
	if (auto idMapping = atosIdToEsminiId.find(ATOSObjectId); idMapping != atosIdToEsminiId.end()) {
		...
		RCLCPP_WARN(
		  me->get_logger(),
		  "Received MONR message for object with ATOS Object ID %d, but no such object exists in the scenario",
		  ATOSObjectId);
	}
}
 ...
 */
ATOS::Trajectory EsminiAdapter::getTrajectoryFromObjectState(uint32_t id, std::vector<SE_ScenarioObjectState>& states) {
 ...
 */
void EsminiAdapter::getObjectStates(double timeStep, std::map<uint32_t, std::vector<SE_ScenarioObjectState>>& states) {
	...
	while (!stopSimulation) {
		...
		bool noMovement =
		  std::all_of(states.begin(), states.end(), [&](auto& pair) { return pair.second.back().speed < 0.1; });
	...
	if (accumTime > MAX_SCENARIO_TIME) {
		...
		RCLCPP_WARN(me->get_logger(),
					"Ran scenario for the minimum time %.2f, possibly no movement in scenario",
					MIN_SCENARIO_TIME);
	}
 ...
 */
std::map<uint32_t, ATOS::Trajectory> EsminiAdapter::extractTrajectories(double timeStep) {
...
void EsminiAdapter::runEsminiSimulation() {
	...
	for (auto& it : esminiIdToTraj) {
		...
		if (auto idMapping = me->objectNameToAtosId.find(objectName); idMapping != me->objectNameToAtosId.end()) {
			...
			std::array<double, 3> llh_0 = {
			  me->testOrigin.position.latitude, me->testOrigin.position.longitude, me->testOrigin.position.altitude};
		...
		for (auto& tp : traj.points) {
			RCLCPP_DEBUG(me->get_logger(),
						 "Trajectory point: %lf, %lf, %lf, %lf, %ld",
						 tp.getXCoord(),
						 tp.getYCoord(),
						 tp.getZCoord(),
						 tp.getHeading(),
...
void EsminiAdapter::onRequestTestOrigin(const std::shared_ptr<atos_interfaces::srv::GetTestOrigin::Request>,
...
int EsminiAdapter::initializeModule() {
	...
	me->objectTrajectoryService = me->create_service<ObjectTrajectorySrv>(
	  ServiceNames::getObjectTrajectory, std::bind(&EsminiAdapter::onRequestObjectTrajectory, _1, _2));
	me->testOriginService = me->create_service<atos_interfaces::srv::GetTestOrigin>(
	  ServiceNames::getTestOrigin, std::bind(&EsminiAdapter::onRequestTestOrigin, _1, _2));
