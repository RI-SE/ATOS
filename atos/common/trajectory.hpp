...
public:
	...
	public:
		typedef enum { CONTROLLED_BY_DRIVE_FILE, CONTROLLED_BY_VEHICLE } ModeType;

		TrajectoryPoint(rclcpp::Logger log) :
		  Loggable(log) {
		...
		void setTime(const std::chrono::duration<Rep, Period>& value) {
			time = value;
		}
		void setTime(const double value) {
			const auto timeVar = static_cast<unsigned long>(value * 1000);
			time			   = std::chrono::milliseconds(timeVar);
		}
		...
		void setZCoord(const double& value) {
			position[2] = value;
		}
		void setHeading(const double& value) {
			heading = std::fmod(value, 2 * M_PI);
			heading += heading < 0 ? 2 * M_PI : 0.0;
		}
		void setVelocity(const Eigen::Vector2d& value) {
			velocity = value;
		}
		void setLongitudinalVelocity(const double& value) {
			velocity[0] = value;
		}
		void setLateralVelocity(const double& value) {
			velocity[1] = value;
		}
		void setAcceleration(const Eigen::Vector2d& value) {
			acceleration = value;
		}
		void setLongitudinalAcceleration(const double& value) {
			acceleration[0] = value;
		}
		void setLateralAcceleration(const double& value) {
			acceleration[1] = value;
		}
		void setCurvature(const double& value) {
			curvature = value;
		}
		void setMode(const ModeType& value) {
			mode = value;
		}
	...
	private:
		...
		Eigen::Matrix<T, rows, 1> zeroNaNs(const Eigen::Matrix<T, rows, 1>& v) const {
	...
	static Trajectory createWilliamsonTurn(double turnRadius,
										   double acceleration,
										   double minSpeed,
										   double maxSpeed,
