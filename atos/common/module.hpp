 ...
 */
template <typename Msg_T, typename MsgData_T> Msg_T msgCtr1(MsgData_T data) {
  Msg_T ret;
  ret.data = data;
  return ret;
}
...
public:
  explicit Module(const std::string &name) : Node(name), getStatusResponsePub(*this){};
  Module() = delete;
  bool shouldExit();
...
protected:
  ...
  static void tryHandleMessage(std::function<void()> tryExecute, std::function<void()> executeIfFail,
                               const std::string &topic, const rclcpp::Logger &logger);
  ...
  bool callService(const std::chrono::duration<double> &timeout, std::shared_ptr<rclcpp::Client<Srv>> &client,
   ...
   */
  template <typename Srv>
  typename std::shared_ptr<rclcpp::Client<Srv>>
  nTimesWaitForService(int n, const std::chrono::duration<double> &timeout, const std::string &serviceName) {
    int retries = 0;
   ...
   */
  template <typename Srv>
  bool nShotServiceRequest(int n, const std::chrono::duration<double> &timeout, const std::string &serviceName,
                           std::shared_ptr<typename Srv::Response> &response) {
