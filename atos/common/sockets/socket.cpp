...
BasicSocket::BasicSocket(const SocketType type, const bool debug) {
...
BasicSocket::BasicSocket(const int sockfd, const bool debug) {
	mSockfd = sockfd;
	setDebug(debug);
}
...
BasicSocket::BasicSocket(const BasicSocket& other) {
...
BasicSocket& BasicSocket::operator=(const BasicSocket& other) {
	if (this != &other) {
		mSockfd = dup(other.mSockfd);
		mDebug	= other.mDebug;
	}
...
sockaddr_in BasicSocket::getLocalAddr() const {
	...
	if (getsockname(mSockfd, reinterpret_cast<struct sockaddr*>(&ret), &addrlen) < 0) {
...
sockaddr_in BasicSocket::getRemoteAddr() const {
	sockaddr_in ret;
	socklen_t addrlen;
	if (getpeername(mSockfd, reinterpret_cast<struct sockaddr*>(&ret), &addrlen) < 0) {
 ...
 */
void BasicSocket::setOption(const SocketOption option, const int value) {
	if (setsockopt(mSockfd, SOL_SOCKET, option, &value, sizeof(value)) < 0) {
 ...
 * \param value Value to set on the option.
 */
void BasicSocket::setOption(const SocketOption option, const struct linger value) {
	if (setsockopt(mSockfd, SOL_SOCKET, static_cast<int>(option), &value, sizeof(value)) < 0) {
		throw SetSockOptError(mSocketOptionNames[option], errno);
	}
}
 ...
 * \param value Value to set on the option.
 */
void BasicSocket::setOption(const FileOption option, const bool value) {
 ...
 */
void BasicSocket::setDebug(const bool enable) {
	mDebug = enable;
}
 ...
 */
void BasicSocket::setReuseAddr(const bool reuseAddr) {
 ...
 */
void BasicSocket::setKeepAlive(const bool keepAlive) {
 ...
 *			(defaults to true).
 */
void BasicSocket::setLinger(const bool linger) {
 ...
 */
void BasicSocket::setLingerSeconds(const int seconds) {
	struct linger lingerOption;
...
BasicSocket::Address BasicSocket::getRemoteIP() const {
	...
	if (inet_ntop(remoteAddr.sin_family, &remoteAddr.sin_addr, ipStr, sizeof(ipStr)) == nullptr) {
 ...
 */
sockaddr_in BasicSocket::toSockaddr(const Address& addr, const Port port) {
	sockaddr_in saddr;
 ...
 */
BasicSocket::HostInfo BasicSocket::fromSockaddr(const struct sockaddr_in& addr) {
 ...
 */
void Socket::send(const std::vector<char>& data, const size_t nBytes, const MessageOption option) {
	if (nBytes > data.size()) {
 ...
 */
bool BasicSocket::haveInput(const double timeout) {
	int status;
	fd_set fds;
	struct timeval tv;
	FD_ZERO(&fds);
 ...
 */
int BasicSocket::preCloseFlush(const std::chrono::system_clock::duration& timeout) {
