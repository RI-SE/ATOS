...
public:
	typedef enum { UDPHANDLER_SUCCESS = 1, UDPHANDLER_FAIL = -1 } ERROR_UDPHANDLER;
	...
	long sendUDP(const std::vector<T>& msg) {
		...
		if (ClientorServer == "Server") {
			...
			std::cout << "\n UDP handler is not defined as Server nor Client, is defined as: " << ClientorServer
					  << std::endl;
			return -1;
		}
		ssize_t res = -1;
	...
	std::string getIPaddr() const {
		return static_cast<std::string>(IPaddr);
	}
