...
Server::Server(const SocketType type, const bool debug) :
  BasicSocket(type, debug) {
	setReuseAddr(true);
}

Server::Server(const SocketType type, const Address& localAddr, const Port port, const bool debug) :
  Server(type, debug) {
 ...
 */
void Server::bind(const SocketType type, const sockaddr_in localAddr) {

	if (::bind(mSockfd, reinterpret_cast<const struct sockaddr*>(&localAddr), sizeof(localAddr)) >= 0) {
		return;
	}
	if (errno == EBADF) {
 ...
 *			to a single remote client.
 */
Socket TCPServer::await(const Address& localAddr, const Port port) {
...
Socket TCPServer::accept() {
	...
	auto conn		  = ::accept(mSockfd, reinterpret_cast<struct sockaddr*>(&cliaddr), &addrlen);
	if (conn < 0) {
 ...
 */
void UDPServer::bind(const HostInfo& localEndpoint) {
...
std::pair<std::vector<char>, BasicSocket::HostInfo> UDPServer::recvfrom() {
	...
	auto bytesRead	   = ::recvfrom(
	  mSockfd, recvBuffer.data(), recvBuffer.size(), 0, reinterpret_cast<struct sockaddr*>(&addr), &socksize);
	if (bytesRead < 0) {
		if (errno == EBADF) {
			throw DisconnectedError();
		}
 ...
 */
void UDPServer::sendto(const std::pair<const std::vector<char>, const HostInfo>& data) {
 ...
 */
void UDPServer::sendto(const std::pair<const std::vector<char>, const HostInfo>& data, const size_t nBytes) {
	if (nBytes > data.first.size()) {
		throw ArgumentError("Bytes to send exceeds data size");
	}
	auto saddr = toSockaddr(data.second);
	auto bytesSent =
	  ::sendto(mSockfd, data.first.data(), nBytes, 0, reinterpret_cast<struct sockaddr*>(&saddr), sizeof(saddr));
	if (bytesSent < 0) {
		if (errno == EBADF) {
			throw DisconnectedError();
		}
...
TCPServer::TCPServer(const bool debug) :
  Server(STREAM, debug) {}
TCPServer::TCPServer(const Address& localAddr, const Port port, const bool debug) :
  Server(STREAM, localAddr, port, debug) {}
UDPServer::UDPServer(const bool debug) :
  Server(DATAGRAM, debug) {}
UDPServer::UDPServer(const Address& localAddr, const Port port, const bool debug) :
