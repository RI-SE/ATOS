...
int UDPHandler::UDPHandlerConnect() {
	// Need to disolve the existing association for the connect call/socket, when using the same client to reconnect to
	// a new IP with the connect or sendto functions. Therefore tempoary change the socket behaviour to AF_UNSPEC,
	// before trying to connect to another IP again. read "man connect" in Unix for more info
	int res = -1;
	struct sockaddr tmp;
