...
public:
	typedef enum { OFF, CLIENT, SERVER } OperationMode;

	typedef enum { SUCCESS = 0, FAILURE = -1 } ErrorCode;

	TCPHandler();
	TCPHandler(int PORT,
			   const std::string IPaddr,
			   std::string ClientorServer = "off",
			   int sockoption			  = 1,
	...
	/**
	 * @brief returns std::string of the private variabel ClientorServer. Tells if class is declared as client or
	 * server.
	 *
	 */
	std::string getClientorServer() const {
	 ...
	 */
	int getConnectionOn() const {
		return static_cast<int>(ConnectionON);
	}
