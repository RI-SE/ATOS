...
bool test_loopback_data() {
	...
	try {
		result = tsrf.wait_for(timeout) == std::future_status::ready && tsrf.get();
	} catch (std::exception& e) {
		std::cout << "TCP server " << e.what() << std::endl;
		result = false;
	}
	try {
		result = tcrf.wait_for(timeout) == std::future_status::ready && tcrf.get();
	} catch (std::exception& e) {
		std::cout << "TCP client " << e.what() << std::endl;
		result = false;
	}
	try {
		result = usrf.wait_for(timeout) == std::future_status::ready && usrf.get();
	} catch (std::exception& e) {
		std::cout << "UDP server " << e.what() << std::endl;
		result = false;
	}
	try {
		result = ucrf.wait_for(timeout) == std::future_status::ready && ucrf.get();
	} catch (std::exception& e) {
		std::cout << "UDP client " << e.what() << std::endl;
		result = false;
	}
...
void run_udp_client(std::promise<bool>& ret, const std::string& remoteIP, const uint16_t port) {
	try {
...
void run_tcp_client(std::promise<bool>& ret, const std::string& remoteIP, const uint16_t port) {
	try {
