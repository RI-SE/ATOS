...
// subscribers to be 0
void waitForSubscriber(std::shared_ptr<rclcpp::Node> node,
	...
	int64_t timeSleptCount = std::chrono::duration_cast<std::chrono::microseconds>(timeSlept).count();
	printf("Waited %" PRId64 " microseconds for the subscriber to %s topic '%s'\n",
		   timeSleptCount,
		   toBeAvailable ? "connect to" : "disconnect from",
		   topicName.c_str());
}

// Sleep for timeout ms or until the service is available to the client
template<typename DurationT, typename ServiceT>
void waitForService(std::shared_ptr<rclcpp::Client<ServiceT>> client, const DurationT& timeout) {
...
void waitForFuture(rclcpp::Executor& executor, std::shared_future<void>& future, const DurationT& timeout) {
	...
	EXPECT_EQ(FutureReturnCode::SUCCESS, futureRet)
	  << "future failed to be set after: " << std::chrono::duration_cast<std::chrono::milliseconds>(elapsedTime).count()
	  << " milliseconds\n";
}
