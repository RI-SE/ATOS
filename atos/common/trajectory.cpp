...
const std::regex Trajectory::fileLinePattern(
  "LINE;(" + RegexPatterns::floatPattern + ");(" + RegexPatterns::floatPattern + ");(" + RegexPatterns::floatPattern +
  ");(" + RegexPatterns::floatPattern + ")?;(" + RegexPatterns::floatPattern + ");(" + RegexPatterns::floatPattern +
  ")?;(" + RegexPatterns::floatPattern + ")?;(" + RegexPatterns::floatPattern + ")?;(" + RegexPatterns::floatPattern +
...
void Trajectory::initializeFromCartesianTrajectory(const atos_interfaces::msg::CartesianTrajectory& traj) {
	...
	for (const auto& tp : traj.points) {
		...
		point.setTime(
		  duration_cast<milliseconds>(seconds{tp.time_from_start.sec} + nanoseconds{tp.time_from_start.nanosec}));
		point.setXCoord(tp.pose.position.x);
		point.setYCoord(tp.pose.position.y);
		point.setZCoord(tp.pose.position.z);
		tf2::Matrix3x3 m(
		  tf2::Quaternion(tp.pose.orientation.x, tp.pose.orientation.y, tp.pose.orientation.z, tp.pose.orientation.w));
...
void Trajectory::initializeFromFile(const std::string& fileName) {
	...
	for (unsigned long lineCount = 0; getline(file, line); lineCount++) {
		if (lineCount == 0) {
			...
			errMsg = "Trajectory line count of file <" + trajFilePath + "> does not match specified line count";
			break;
		} else if (lineCount == nPoints + 1) {
			...
			if (regex_search(line, match, fileLinePattern)) {
				...
				errMsg = "Line " + to_string(lineCount) + " of trajectory file <" + trajFilePath + "> badly formatted";
				break;
			}
		}
	}
 ...
 * \return
 */
Trajectory::TrajectoryPoint Trajectory::TrajectoryPoint::relativeTo(const TrajectoryPoint& other) const {
	...
	relative.setPosition(R3 * (zeroNaNs(this->getPosition()) - zeroNaNs(other.getPosition())));
	...
	if (rPrim3.norm() > 0.001) {
		relative.setCurvature(rPrim3.cross(rBis3).norm() / std::pow(rPrim3.norm(), 3));
	} else {
...
Trajectory Trajectory::relativeTo(const Trajectory& other) const {
	using namespace Eigen;
	...
	for (auto trajPt = this->points.begin(); trajPt != this->points.end(); ++trajPt) {
		auto nearestTrajPtInOther = getNearest(
		  other.points.begin(), other.points.end(), std::chrono::duration<double>(trajPt->getTime()).count());
...
Trajectory::const_iterator Trajectory::getNearest(const_iterator first, const_iterator last, const double& time) {
	...
	return (std::chrono::duration<double>(after->getTime()).count() - time) <
			   (time - std::chrono::duration<double>(before->getTime()).count())
			 ? after
			 : before;
}
 ...
 */
Trajectory Trajectory::appendedWith(const Trajectory& other) {
	if (!other.isValid()) {
	...
	auto secondTrajStartTime =
	  newTrajectory.points.front().getTime(); // TODO maybe a time offset between the two trajectories?

	std::transform(
	  other.points.begin(), other.points.end(), std::back_inserter(newTrajectory.points), [&](TrajectoryPoint otherPt) {
...
/*!
 * \brief Trajectory::TrajectoryPoint::rescaleToVelocity Returns a copy of the trajectory rescaled to match a certain
 * constant speed.
 * \param vel_m_s Speed to which trajectory is to be reduced
 * \return Trajectory
 */
Trajectory Trajectory::rescaledToVelocity(const double vel_m_s) const {
	...
	Eigen::Vector2d maxVel_m_s = std::max_element(newTrajectory.points.begin(),
												  newTrajectory.points.end(),
												  [](const TrajectoryPoint& pt1, const TrajectoryPoint& pt2) {
													  return pt1.getVelocity().norm() < pt2.getVelocity().norm();
												  })
								   .base()
								   ->getVelocity();
...
Trajectory Trajectory::createWilliamsonTurn(double turnRadius,
											double acceleration,
											double minSpeed,
											double maxSpeed,
	...
	auto totalRuntime =
	  accelerationPeriod + topSpeedPeriod + accelerationPeriod; // Accelerate -> Top Speed -> Decelerate
	...
	RCLCPP_DEBUG(
	  startPoint.get_logger(), "curvatureArray rows: %ld, cols: %ld", curvatureArray.rows(), curvatureArray.cols());
	...
	curvatureArray << -1 / radius * Eigen::ArrayXd::Ones(n0), 1 / radius * Eigen::ArrayXd::Ones(n1),
	  Eigen::ArrayXd::Zero(n2);
	...
	retval.name	  = "Williamson_x" + std::to_string(startPoint.getXCoord()) + "_y" +
				  std::to_string(startPoint.getYCoord()) + "_z" + std::to_string(startPoint.getZCoord()) + "_hdg" +
				  std::to_string(headingRad * 180.0 / M_PI);
	retval.id	   = 0;
	retval.version = 0;
	return retval;
}
...
void Trajectory::saveToFile(const std::string& fileName) const {
	...
	try {
		...
		outputTraj << "TRAJECTORY;" << this->id << ";" << this->name << ";" << this->version << ";"
				   << this->points.size() << ";" << "\n";
		for (const auto& point : points) {
			outputTraj << "LINE;" << std::fixed << std::setprecision(2)
					   << std::chrono::duration<double>(point.getTime()).count() << ";" << std::fixed
					   << std::setprecision(6) << point.getXCoord() << ";" << std::fixed << std::setprecision(6)
					   << point.getYCoord() << ";" << std::fixed << std::setprecision(6) << point.getZCoord() << ";"
					   << std::fixed << std::setprecision(6) << point.getHeading() << ";" << std::fixed
					   << std::setprecision(6) << point.getLongitudinalVelocity() << ";"
					   << ";" // point.getLateralVelocity() << ";"
					   << std::fixed << std::setprecision(6) << (point.getLongitudinalAcceleration()) << ";"
					   << ";" // point.getLateralAcceleration() << ";"
					   << std::fixed << std::setprecision(6) << point.getCurvature() << ";" << std::fixed
					   << std::setprecision(6) << point.getMode() << ";ENDLINE;" << "\n";
		}
