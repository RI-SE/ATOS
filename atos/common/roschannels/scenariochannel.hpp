...
public:
	Pub(rclcpp::Node& node) :
	  BasePub<message_type>(node, topicName, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local()) {}
};

class Sub : public BaseSub<message_type> {
public:
	Sub(rclcpp::Node& node, std::function<void(const message_type::SharedPtr)> callback) :
	  BaseSub<message_type>(node, topicName, callback, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local()) {}
};
} // namespace StoryBoardElementStateChange
} // namespace ROSChannels
