...
int JournalRecordMonitorData(const ObjectDataType* objectData) {
	...
	fp = beginJournalEntry();
	if (fp != nullptr) {
		data->isTimestampValid
		  ? fprintf(fp, "%.6f" DELIMITER, data->timestamp.tv_sec + data->timestamp.tv_usec / 1000000.0)
		...
		data->position.isPositionValid ? fprintf(fp,
												 "%.3f" DELIMITER "%.3f" DELIMITER "%.3f" DELIMITER,
												 data->position.xCoord_m,
												 data->position.yCoord_m,
												 data->position.zCoord_m)
