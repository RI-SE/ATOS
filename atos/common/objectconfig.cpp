...
ObjectConfig::ObjectConfig(rclcpp::Logger lg) :
  Loggable(lg),
  trajectory(lg) {
...
void ObjectConfig::parseObjectIdFromConfigurationFile(const fs::path& objectFile) {
	...
	// Get ID setting
	if (UtilGetObjectFileSetting(
		  OBJECT_SETTING_ID, objectFile.c_str(), objectFile.string().length(), setting, sizeof(setting)) == -1) {
	...
	if (endptr == setting) {
		throw std::invalid_argument("ID " + std::string(setting) + " in file " + objectFile.string() + " is invalid");
	}
	this->transmitterID = id;
}

void ObjectConfig::parseConfigurationFile(const fs::path& objectFile) {
	...
	// Get IP setting
	if (UtilGetObjectFileSetting(
		  OBJECT_SETTING_IP, objectFile.c_str(), objectFile.string().length() + 1, setting, sizeof(setting)) == -1) {
	...
	if (result == -1) {
		...
		throw std::invalid_argument("Address " + std::string(setting) + " in object file " + objectFile.string() +
	...
	// Get anchor setting
	if (UtilGetObjectFileSetting(
		  OBJECT_SETTING_IS_ANCHOR, objectFile.c_str(), objectFile.string().length(), setting, sizeof(setting)) == -1) {
	...
	if (UtilGetObjectFileSetting(
		  OBJECT_SETTING_TRAJ, objectFile.c_str(), objectFile.string().length(), setting, sizeof(setting)) == 0) {

		fs::path trajFile(std::string(trajDirPath) + std::string(setting));
		if (!fs::exists(trajFile.string())) {
			throw std::invalid_argument("Configured trajectory file " + std::string(setting) + " in file " +
										objectFile.string() + " not found");
		}
	...
	if (UtilGetObjectFileSetting(
		  OBJECT_SETTING_OPENDRIVE, objectFile.c_str(), objectFile.string().length(), setting, sizeof(setting)) == 0) {

		fs::path odrFile(std::string(odrDirPath) + std::string(setting));
		if (!fs::exists(odrFile.string())) {
			throw std::invalid_argument("Configured OpenDRIVE file " + std::string(setting) + " in file " +
										objectFile.string() + " not found");
		}
	...
	if (UtilGetObjectFileSetting(
		  OBJECT_SETTING_OPENSCENARIO, objectFile.c_str(), objectFile.string().length(), setting, sizeof(setting)) ==
		0) {

		fs::path oscFile(std::string(oscDirPath) + std::string(setting));
		if (!fs::exists(oscFile.string())) {
			throw std::invalid_argument("Configured OpenSCENARIO file " + std::string(setting) + " in file " +
										objectFile.string() + " not found");
		}
	...
	this->origin = {};
	if (UtilGetObjectFileSetting(
		  OBJECT_SETTING_ORIGIN_LATITUDE, objectFile.c_str(), objectFile.string().length(), setting, sizeof(setting)) !=
		-1) {
	...
	if (UtilGetObjectFileSetting(OBJECT_SETTING_ORIGIN_LONGITUDE,
								 objectFile.c_str(),
	...
	if (UtilGetObjectFileSetting(
		  OBJECT_SETTING_ORIGIN_ALTITUDE, objectFile.c_str(), objectFile.string().length(), setting, sizeof(setting)) !=
		-1) {
	...
	if (origin.isAltitudeValid == origin.isLatitudeValid && origin.isLatitudeValid == origin.isLongitudeValid) {
	...
	if (UtilGetObjectFileSetting(OBJECT_SETTING_TURNING_DIAMETER,
								 objectFile.c_str(),
								 objectFile.string().length(),
								 setting,
								 sizeof(setting)) != -1) {
		...
		if (endptr == setting) {
			throw std::invalid_argument("Turning diameter " + std::string(setting) + " in file " + objectFile.string() +
										" is invalid");
		}
	...
	// Get Maximum speed
	if (UtilGetObjectFileSetting(
		  OBJECT_SETTING_MAX_SPEED, objectFile.c_str(), objectFile.string().length(), setting, sizeof(setting)) != -1) {
		char* endptr;
		double val = strtod(setting, &endptr);

		if (endptr == setting) {
			throw std::invalid_argument("Max speed " + std::string(setting) + " in file " + objectFile.string() +
										" is invalid");
		}
	...
	if (UtilGetObjectFileSetting(OBJECT_SETTING_IS_OSI_COMPATIBLE,
								 objectFile.c_str(),
								 objectFile.string().length(),
								 setting,
								 sizeof(setting)) == -1) {
	...
	// Get Injector IDs
	if (UtilGetObjectFileSetting(
		  OBJECT_SETTING_INJECTOR_IDS, objectFile.c_str(), objectFile.string().length(), setting, sizeof(setting)) !=
		-1) {
		std::vector<int> ids;
