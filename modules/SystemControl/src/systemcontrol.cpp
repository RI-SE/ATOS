...
void SystemControl::onAbortMessage(const std_msg::Empty::SharedPtr){}

void SystemControl::onAllClearMessage(const std_msg::Empty::SharedPtr){}

void SystemControl::onFailureMessage(const std_msg::UInt8::SharedPtr msg){
...
void SystemControl::onGetStatusResponse(const std_msg::String::SharedPtr msg){
	SystemControlGetStatusMessage(msg->data.c_str(), 0);
}

void SystemControl::onBackToStartResponse(const std_msg::Int8::SharedPtr msg){
...
void SystemControl::receiveUserCommand()
{
	if (SystemControlState == SERVER_STATE_ERROR) {
		this->abortPub.publish(std_msg::Empty());
		return;
	}
	...
	if (ModeU8 == 0) {
		if (ClientSocket <= 0) {
			if (SystemControlState == SERVER_STATE_UNDEFINED) {
				...
				this->dataDictionaryPub.publish(std_msg::Empty());
...
void SystemControl::processUserCommand()
{
	...
	case InitializeScenario_0:
		if (SystemControlState == SERVER_STATE_IDLE && objectControlState == OBC_STATE_IDLE) {
			try{
				this->initPub.publish(std_msg::Empty());
			}
			catch(...){
	...
	case ConnectObject_0:
		if (SystemControlState == SERVER_STATE_IDLE && objectControlState == OBC_STATE_INITIALIZED) {
			try{
				this->connectPub.publish(std_msg::Empty());
			}
			catch(...){
	...
	case DisconnectObject_0:
		if (SystemControlState == SERVER_STATE_IDLE) {
			try{
				this->disconnectPub.publish(std_msg::Empty());
			}
			catch(...){
	...
	case ArmScenario_0:
		if (SystemControlState == SERVER_STATE_IDLE && objectControlState == OBC_STATE_CONNECTED) {
			SystemControlState = SERVER_STATE_INWORK;
			try{
				this->armPub.publish(std_msg::Empty());
			}
			catch(...){
		...
		if (CurrentInputArgCount == CommandArgCount) {
			if (SystemControlState == SERVER_STATE_IDLE
				...
				if (!strcasecmp(SystemControlArgument[0], ENABLE_COMMAND_STRING)
					...
					this->remoteControlEnablePub.publish(std_msg::Empty());
					...
					this->remoteControlDisablePub.publish(std_msg::Empty());
					responseCode = SYSTEM_CONTROL_RESPONSE_CODE_OK;
				}
				else {
	...
	case RemoteControlManoeuvre_2:
		if (CurrentInputArgCount == CommandArgCount) {
			if (SystemControlState == SERVER_STATE_IDLE && objectControlState == OBC_STATE_REMOTECTRL) {
				auto msg = maestro_msg::ManoeuvreCommand();
	...
	case SetObjectEnableStatus_2:
		if (CurrentInputArgCount == CommandArgCount) {
			if (SystemControlState == SERVER_STATE_IDLE && objectControlState != OBC_STATE_RUNNING) {
				auto msg = maestro_msg::ObjectEnabled();
				if (inet_pton(AF_INET, SystemControlArgument[0], &(msg.object_ip)) != -1) {
					responseCode = SYSTEM_CONTROL_RESPONSE_CODE_OK;
					switch (atoi(SystemControlArgument[1])) {
					case OBJECT_ENABLED:
		...
		if (CurrentInputArgCount == CommandArgCount) {
			if (SystemControlState == SERVER_STATE_IDLE && objectControlState == OBC_STATE_ARMED)	//Temporary!
			{
				...
				RCLCPP_INFO(get_logger(), "Sending START <%s> (delayed +%u ms)", pcBuffer,
							DelayedStartU32);
				try{
					this->startPub.publish(std_msg::Empty());
				}
				catch(...){
	...
	case stop_0:
		try{
			this->stopPub.publish(std_msg::Empty());
			SystemControlState = SERVER_STATE_IDLE;
			SystemControlCommand = Idle_0;
			bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
		...
		catch(...){this->disconnectPub.publish(std_msg::Empty());
	...
	case ClearAllScenario_0:
		try{
			this->allClearPub.publish(std_msg::Empty());
			SystemControlState = SERVER_STATE_IDLE;
			SystemControlCommand = Idle_0;
			if (ClientSocket >= 0) {
				bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
	...
	case Exit_0:
		try{
			this->exitPub.publish(std_msg::Empty());
			iExit = 1;
			usleep(1000000);
			SystemControlCommand = Idle_0;
			bzero(ControlResponseBuffer, SYSTEM_CONTROL_CONTROL_RESPONSE_SIZE);
...
I32 SystemControl::SystemControlGetStatusMessage(const char *respondingModule, U8 debug) {
	...
	case GETSTATUS_SEND:
		...
		if (timercmp(&getStatusSendTimer, &currentSystemTime, <)) {
			...
			//Send getstatus
			this->getStatusPub.publish(std_msg::Empty());
